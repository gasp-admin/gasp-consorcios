...
const EST_COLOR = {
  solicitada: AM, pendiente_pago: AM, confirmada: VD,
  rechazada: RJ, cancelada: GR, expirada: GR,
}
const EST_LABEL = {
  solicitada: 'Solicitada', pendiente_pago: 'Pend. pago', confirmada: 'Confirmada',
  rechazada: 'Rechazada', cancelada: 'Cancelada', expirada: 'Expirada',
}
...
export default function SUM() {
  ...
  const [form, setForm]         = useState(null)   // form de espacio
  const [dForm, setDForm]       = useState(null)   // form de franja
  ...
  const cargarDetalle = useCallback(async () => {
    ...
    const [{ data: d }, { data: r }] = await Promise.all([
      supabase.from('con_sum_disponibilidad').select('*').eq('espacio_id', espSel.id)
        .order('dia_semana').order('hora_inicio'),
      supabase.from('con_sum_reservas').select('*').eq('espacio_id', espSel.id)
  ...
  async function guardarEspacio() {
    ...
    const payload = {
      ...
      registrar_ingreso_caja: !!form.registrar_ingreso_caja,
      auto_confirmar_pago: !!form.auto_confirmar_pago,
    ...
    if (form.id) {
      ...
      const { error } = await supabase.from('con_sum_espacios').insert([{
        id: `SUM-${consorcioId}-${Date.now()}`, admin_id: uid, consorcio_id: consorcioId, ...payload,
      }])
      if (error) return setMsg({ tipo:'error', texto:error.message })
  ...
  async function agregarFranja() {
    ...
    const { error } = await supabase.from('con_sum_disponibilidad').insert([{
      ...
      dia_semana: parseInt(dForm.dia), franja_label: dForm.label || 'Franja',
      hora_inicio: dForm.hi, hora_fin: dForm.hf, activo: true,
    }])
    if (error) return setMsg({ tipo:'error', texto:error.message })
  ...
  async function borrarFranja(id) {
    ...
    await supabase.from('con_sum_disponibilidad').delete().eq('id', id)
    cargarDetalle()
  }
  ...
  async function aprobar(r) {
    ...
    const { error } = await supabase.from('con_sum_reservas')
      .update({ estado, pago_estado: r.pago_requerido ? 'pendiente' : 'no_aplica' }).eq('id', r.id)
    if (error) return setMsg({ tipo:'error', texto:error.message })
  ...
  async function confirmarPago(r) {
    ...
    if (error) return setMsg({ tipo:'error', texto:error.message })
    setMsg({ tipo:'ok', texto: espSel?.registrar_ingreso_caja
      ? '✓ Pago confirmado — ingreso cargado en Movimientos varios (Uso Amenities)'
      : '✓ Pago confirmado' })
    cargarDetalle()
  }
  ...
  // ── Bloqueo del admin (cierra un día/franja) ──
  async function guardarBloqueo() {
    ...
    const { error } = await supabase.from('con_sum_reservas').insert([{
      ...
      unidad_id: null, tipo: 'bloqueo', fecha: bForm.fecha, inicio, fin,
      franja_label: bForm.label || 'Bloqueo', estado: 'confirmada', creado_por: 'admin',
      notas: bForm.notas || 'Bloqueo administrativo',
    }])
    if (error) return setMsg({ tipo:'error', texto: error.message.includes('sum_sin_solape')
      ? 'Ese día/horario ya está ocupado o reservado' : error.message })
  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
        ...
        <Btn small color={AZ} onClick={()=>setForm({ activo:true, requiere_pago:false, requiere_aprobacion:false,
          registrar_ingreso_caja:false, auto_confirmar_pago:false, tarifa:0, anticipacion_max_dias:60, max_reservas_activas_uf:1 })}>
          + Nuevo espacio
        </Btn>
      </div>
      <Msg data={msg} />
      ...
      {espacios.length > 0 && (
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:12 }}>
          {espacios.map(e => (
            <div key={e.id} onClick={()=>setEspSel(e)} style={{ cursor:'pointer', padding:'6px 12px', borderRadius:8,
              border:`1.5px solid ${espSel?.id===e.id?AZ:'#e5e7eb'}`, background: espSel?.id===e.id?'#eff6ff':'#fff', fontSize:13 }}>
        ...
        <Card style={{ marginBottom:16, border:`1.5px solid ${AZ}` }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:12, marginBottom:12 }}>
            <div>
              ...
              <Input value={form.nombre||''} placeholder="SUM, Quincho, Parrilla…" onChange={e=>setForm(f=>({...f,nombre:e.target.value}))} />
            </div>
            <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
            <div>
              ...
              <Input type="number" value={form.anticipacion_max_dias} onChange={e=>setForm(f=>({...f,anticipacion_max_dias:e.target.value}))} />
            </div>
            <div>
              <div style={{ fontSize:12, color:GR, marginBottom:4 }}>Reservas activas máx. por UF</div>
              <Input type="number" value={form.max_reservas_activas_uf} onChange={e=>setForm(f=>({...f,max_reservas_activas_uf:e.target.value}))} />
            </div>
          </div>
          ...
          <Card style={{ marginBottom:16 }}>
            ...
            {dForm && (
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center', marginBottom:12, background:'#f8fafc', padding:10, borderRadius:8 }}>
                <Sel value={dForm.dia} onChange={e=>setDForm(f=>({...f,dia:e.target.value}))}>
                  <option value="">Día…</option>
                  {DIAS.map((d,i)=><option key={i} value={i}>{d}</option>)}
                </Sel>
                <Sel value={dForm.label} onChange={e=>{ const p=FRANJAS_PRESET.find(x=>x.label===e.target.value); setDForm(f=>({...f,label:e.target.value, hi:p?p.hi:f.hi, hf:p?p.hf:f.hf})) }}>
                  {FRANJAS_PRESET.map(p=><option key={p.label} value={p.label}>{p.label}</option>)}
                </Sel>
                <input type="time" value={dForm.hi} onChange={e=>setDForm(f=>({...f,hi:e.target.value}))} style={{ padding:'6px 8px', border:'1px solid #d1d5db', borderRadius:6 }} />
                <span style={{ color:GR }}>a</span>
                <input type="time" value={dForm.hf} onChange={e=>setDForm(f=>({...f,hf:e.target.value}))} style={{ padding:'6px 8px', border:'1px solid #d1d5db', borderRadius:6 }} />
          ...
          <Card>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10, flexWrap:'wrap', gap:8 }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                <Sel value={fEstado} onChange={e=>setFEstado(e.target.value)}>
            ...
            {bForm && (
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center', marginBottom:12, background:'#fff1f2', padding:10, borderRadius:8 }}>
                <input type="date" value={bForm.fecha} onChange={e=>setBForm(f=>({...f,fecha:e.target.value}))} style={{ padding:'6px 8px', border:'1px solid #d1d5db', borderRadius:6 }} />
                <input type="time" value={bForm.hi} onChange={e=>setBForm(f=>({...f,hi:e.target.value}))} style={{ padding:'6px 8px', border:'1px solid #d1d5db', borderRadius:6 }} />
                <span style={{ color:GR }}>a</span>
                <input type="time" value={bForm.hf} onChange={e=>setBForm(f=>({...f,hf:e.target.value}))} style={{ padding:'6px 8px', border:'1px solid #d1d5db', borderRadius:6 }} />
                <Input value={bForm.notas||''} placeholder="Motivo (opcional)" onChange={e=>setBForm(f=>({...f,notas:e.target.value}))} />
                    ...
                    <tbody>
                      {reservasFiltradas.map(r => (
                        <tr key={r.id} style={{ borderBottom:'1px solid #f1f5f9' }}>
                          ...
                          <td><Badge color={EST_COLOR[r.estado] || GR}>{EST_LABEL[r.estado] || r.estado}</Badge></td>

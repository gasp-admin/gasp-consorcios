...
export default function RendicionCuentas() {
  const { session, consorcioId, consorcioActivo, expensas, copropietarios, unidades } = useApp()
  ...
  async function generar() {
    ...
    const [{ data: gastos }, { data: cobranzas }, { data: sueldos }] = await Promise.all([
      supabase.from('con_gastos').select('*').eq('consorcio_id', consorcioId).eq('expensa_id', expSel).eq('admin_id', uid),
      supabase.from('con_cobranzas').select('*').eq('consorcio_id', consorcioId).eq('expensa_id', expSel).eq('admin_id', uid).in('estado',['acreditado','cobrado']),
      supabase.from('con_sueldos').select('*').eq('consorcio_id', consorcioId).eq('expensa_id', expSel).eq('admin_id', uid),
    ])
...
<div class="wrap">
  ...
  <div class="footer">
    ...
    await supabase.from('con_rendicion_cuentas').insert([{
      id: rendId, admin_id: uid, consorcio_id: consorcioId,

...
export default function HistorialLiquidaciones() {
  ...
  // ── Flujo principal ────────────────────────────────────────────────────────
  // El servidor determina si es grande o no — el frontend no asume nada
  const procesarPDF = async (file, colaId) => {
    ...
    try {
      // ── Paso 1: Meta (el servidor también devuelve total_ufs y formato) ────
      ...
      if (!metaRes.ok) {
        setMsg(`❌ Error meta: ${metaRes.error || 'desconocido'}`)
        return
      }
      ...
      setMsg(`✅ Meta OK — Período ${periodo} — ${totalUFs} UFs — Formato: ${formato}`)

      if (!esGrande) {
        // Consorcio pequeño: el meta ya procesó el prorrateo en procesar_pdf
        // No hace falta procesar rangos
      ...
      for (let i = 0; i < rangos.length; i++) {
        ...
        if (!res.ok) {
          setMsg(`⚠️ Rango ${desde}–${hasta}: ${res.error || 'error'}`)
        } else {
          ufsOk += res.nUFs || 0
          ufsErr += res.errores || 0
        }
      ...
      await efPost({ accion: 'finalizar', cola_id: colaId, expensa_id, consorcio_id: con.id })
        .catch(e => console.warn('finalizar:', e))
    ...
    <div style={{ padding: 20, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', color: '#6b7280', fontSize: 13 }}>Mis Expensas v36 — Modo por consorcio</p>
        </div>
      ...
      {msg && (
        <div style={{
          ...
          border: '1px solid',
          borderColor: msg.startsWith('✅') ? '#6ee7b7' : msg.startsWith('❌') ? '#fca5a5' : '#fde68a',
      ...
      {tab === 'importar' && (
        <div>
          ...
          <div style={{
            ...
            {consorcioImportar
              ? <div style={{ marginTop: 10, padding: '8px 14px', background: '#dcfce7', borderRadius: 7, fontSize: 13, color: '#166534', fontWeight: 600, display: 'flex', gap: 8, alignItems: 'center' }}>
                  ✅ <b>{consorcioImportar.nombre}</b>
                  <span style={{ fontWeight: 400, fontSize: 11 }}>({consorcioImportar.id})</span>
                </div>
          ...
          <div style={{ background: '#fffbeb', border: '2px solid #f59e0b', borderRadius: 8, padding: 16, marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: '#92400e', margin: '0 0 10px' }}>
              El sistema detecta automáticamente el formato del consorcio (standard / con_subtotal) y el tamaño. Sin límite de UFs.
            </p>
            <label style={{
              display: 'inline-flex', alignItems: 'center', gap: 8, padding: '9px 20px',
              background: consorcioImportar && !procesando ? '#f59e0b' : '#d1d5db',
              color: '#fff', borderRadius: 6, fontSize: 13, fontWeight: 700,
              cursor: consorcioImportar && !procesando ? 'pointer' : 'not-allowed',
              opacity: consorcioImportar && !procesando ? 1 : 0.6
            }}>
              📁 Elegir PDF
            ...
            {!consorcioImportar && (
              <p style={{ margin: '8px 0 0', fontSize: 12, color: '#dc2626', fontWeight: 600 }}>⛔ Seleccioná el consorcio destino primero.</p>
            )}
          </div>

          {/* Info formatos */}
          <div style={{ ...card, background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
            <h4 style={{ margin: '0 0 8px', fontSize: 13, color: '#166534' }}>✅ Formatos soportados (v36)</h4>
            <ul style={{ margin: 0, paddingLeft: 20, fontSize: 12, color: '#166534', lineHeight: 1.8 }}>
              <li><b>standard</b>: columnas estándar de Mis Expensas — Mejillón, Caracol y la mayoría</li>
              <li><b>con_subtotal</b>: tiene columna SUBTOTAL antes de RED./AJUSTES — Ática II</li>
              <li>Para configurar el formato de un consorcio nuevo, usar la acción <code>get_formato</code></li>
            </ul>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          {loading ? (
            ...
            <div style={{ ...card, textAlign: 'center', color: '#6b7280', padding: 40 }}>
              ...
              No hay liquidaciones importadas.
            </div>
          ) : (

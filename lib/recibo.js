...
// Numeración (A2, 2026-09-24): nro_recibo correlativo por consorcio (ej. 1285-000001), asignado por la base
// al registrar el pago (trigger fn_asignar_nro_recibo). Pagos anteriores conservan su identificador previo.
export function numeroRecibo(cob) {
  return trim(cob?.nro_recibo) || trim(cob?.recibo_numero) || String(cob?.id || '').slice(-8).toUpperCase()
}
// Referencia bancaria / de operación (recibo_numero) — se muestra aparte cuando el recibo ya tiene número propio.
export function referenciaPago(cob) {
  const r = trim(cob?.recibo_numero)
  return r && trim(cob?.nro_recibo) && r !== trim(cob?.nro_recibo) ? r : ''
}
...
${row('CUIT', esc(trim(con.cuit) || 'En trámite'))}
...
${row('Medio de pago', esc(medio))}${referenciaPago(cob) ? row('Ref. de pago', esc(referenciaPago(cob))) : ''}${cob?.canal_cobro ? row('Canal', esc(cob.canal_cobro)) : ''}

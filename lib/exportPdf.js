...
export function generarPDFLiquidacion({ consorcioActivo, expensa, gastos, detalles, unidades, copropietarios, adminPerfil, lufsHist }) {
  ...
  const ufsTabla=detalles.map(det=>{
    ...
    return { uf:u.nro_uf_pdf||u.numero_interno||u.numero||det.unidad_id, dpto:u.numero||(u.piso?`${u.piso} ${u.numero||''}`.trim():(u.tipo||'')), prop:cp.apellido_nombre||'—', salAnt, pagos, deuda, interes, pct:pctGrales, fdoUF, gralesUF, cochUF, dptUF, total, redondeo:parseFloat(det.redondeo)||0 }
  }).sort((a,b)=>{

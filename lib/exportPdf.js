...
export function generarPDFLiquidacion({ consorcioActivo, expensa, gastos, detalles, unidades, copropietarios, adminPerfil, lufsHist }) {
  ...
  const sumGralesPct = (unidades||[]).reduce((a,x)=>a+(parseFloat(x.pct_gtos_grales)||0),0)
  const ufsTabla=detalles.map(det=>{
    ...
    let fdoUF=0, gralesUF=0, cochUF=0, dptUF=0, pctGrales=0
    if (esHistorico) {
      ...
      pctGrales = sumGralesPct>0 ? ((parseFloat(u.pct_gtos_grales)||0)/sumGralesPct)*100 : 0
  ...
  // Pendiente de pago = egresos cargados y NO pagados (con_gastos.pagado===false). Criterio devengado:
  // el gasto igual cuenta como egreso del período; acá sólo se informa el saldo aún no salido de caja.
  // NO se toca saldo_caja_final ni total_egresos. (Antes: por error sumaba la DEUDA de las UF/morosos.)
  const totPend=(gastos||[]).reduce((a,g)=>a+((g.pagado===false)?(parseFloat(g.monto)||0):0),0)
  ...
  const tots2={salAnt:ufsTabla.reduce((a,u)=>a+u.salAnt,0),pagos:ufsTabla.reduce((a,u)=>a+u.pagos,0),deuda:ufsTabla.reduce((a,u)=>a+u.deuda,0),salNetoSum:ufsTabla.reduce((a,u)=>a+u.salNeto,0),deudaBSum:ufsTabla.reduce((a,u)=>a+u.deudaB,0),interes:ufsTabla.reduce((a,u)=>a+u.interes,0),fdoUF:ufsTabla.reduce((a,u)=>a+u.fdoUF,0),gralesUF:ufsTabla.reduce((a,u)=>a+u.gralesUF,0),cochUF:ufsTabla.reduce((a,u)=>a+u.cochUF,0),dptUF:ufsTabla.reduce((a,u)=>a+u.dptUF,0),total:ufsTabla.reduce((a,u)=>a+u.total,0)}
  const pagsProrr=chunks.map((chunk,ci)=>{
    const np=4+ci; const esUlt=ci===chunks.length-1
    const filas=chunk.map(u=>`<tr><td class="L">${String(u.uf).padStart(2,'0')}</td><td class="L">${u.dpto}</td><td class="L" style="max-width:70px;overflow:hidden;white-space:nowrap">${u.prop}</td><td>${fmtN(u.salAnt)}</td><td>${fmtN(u.pagos)}</td><td>${esHistorico?(u.deuda>0?fmtN(u.deuda):'0,00'):(u.deudaB!==0?fmtN(u.deudaB):'0,00')}</td><td>${u.interes>0?fmtN(u.interes):'0,00'}</td><td>${u.pct.toFixed(2)}%</td><td style="font-size:5.5pt">0,00%</td><td>${fmtN(u.fdoUF)}</td><td style="font-size:5.5pt">0,00%</td><td>${fmtN(u.gralesUF)}</td><td style="font-size:5.5pt">0,00%</td><td>${fmtN(u.cochUF)}</td><td>${fmtN(u.dptUF)}</td><td style="font-size:5.5pt">${fmtN(u.redondeo)}</td><td style="font-weight:600">${fmtN(u.total)}</td><td class="L" style="font-size:5.5pt;color:#888">${String(u.uf).padStart(2,'0')}</td></tr>`).join('')
    const filaTot=esUlt?`<tr class="gt"><td colspan="3" style="text-align:right">TOTAL</td><td>${fmtN(tots2.salAnt)}</td><td>${fmtN(tots2.pagos)}</td><td>${fmtN(esHistorico?tots2.deuda:tots2.deudaBSum)}</td><td>${fmtN(tots2.interes)}</td><td>100%</td><td>,00</td><td>${fmtN(tots2.fdoUF)}</td><td>100%</td><td>${fmtN(tots2.gralesUF)}</td><td>100%</td><td>${fmtN(tots2.cochUF)}</td><td>${fmtN(tots2.dptUF)}</td><td>,00</td><td>${fmtN(tots2.total)}</td><td></td></tr>`:''

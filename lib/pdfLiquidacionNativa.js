// lib/pdfLiquidacionNativa.js
// U1 (2026-09-24) — Plantilla ÚNICA de liquidación para consorcios NATIVOS:
// "EXPENSAS PROVINCIA DE BUENOS AIRES Ley 14.701".
// Traslado textual del generador de "Vista previa" de modules/expensas/LiquidacionPeriodo.jsx (líneas 809-1266 de
// la versión 2026-09-24). No cambia el HTML: recibe por parámetro los mismos valores que antes tomaba del estado
// del componente, con los mismos nombres. Los datos son serializables → U2 los guarda al cierre (snapshot).
// Consorcios HISTÓRICOS: NO usan esta plantilla (su PDF válido es el del sistema de origen).
// U4 (2026-09-25) — art. 11 Ley 14.701: datos del administrador desde con_admin_perfil (no escritos en código),
//   CUIT del consorcio "En trámite", domicilio del consorcio, bloque de honorarios (inc. e), bloque de seguro
//   con coberturas y cuota del período (inc. j), % de rubro con coma. En preliquidación (esPreliquidacion=true)
//   avisa lo que falta cargar. Montos, prorrateo y estado financiero SIN cambios.

import { periodoLabel } from './formatters'
...
export function resolverCodigosColumna(rawCodigos, colsActivas) {
  const byLower = {}
...
/**
 * Datos de entrada (mismos nombres que el estado de LiquidacionPeriodo):
 * { consorcioActivo, expSel, gastos, distribucion, columnasLiq, gruposLiq, config, importesPorColumna,
 *   saldoCajaAnterior, cobradoActual, cobradoAdeudado, cobradoInteres, notasPeriodo }
 */
export function construirHTMLLiquidacionNativa(datos) {
  const { consorcioActivo, expSel, gastos, distribucion, columnasLiq, gruposLiq, config, importesPorColumna,
    saldoCajaAnterior, cobradoActual, cobradoAdeudado, cobradoInteres, notasPeriodo,
    adminPerfil, esPreliquidacion } = datos || {}
  // ── U4: datos del administrador y del consorcio (art. 11 incs. b y c) ──
  const escH = v => String(v ?? '').replace(/[&<>"]/g, c => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;' }[c]))
  const fDMY = d => { const m = String(d || '').match(/^(\d{4})-(\d{2})-(\d{2})/); return m ? `${m[3]}/${m[2]}/${m[1]}` : '—' }
  const admP = adminPerfil || {}
  const admNombre = String(admP.nombre || 'Javier García Pérez').trim()
  const admDom    = String(admP.direccion || 'Del Lenguado 1313 Loc 3').trim()
  const admEmail  = String(admP.email || 'administracion@administracionpinamar.com').trim()
  const admCuit   = String(admP.cuit || '20-18600680-2').trim()
  const admRpac   = String(admP.matricula_rpac || '83').trim().replace(/^N[°º.]?\s*/i, '')
  const admTel    = String(admP.telefono || '').trim()
  const admFiscal = String(admP.situacion_fiscal || '').trim()
  const cuitCon   = String(consorcioActivo?.cuit || '').trim() || 'En trámite'
  const domCon    = [consorcioActivo?.direccion, consorcioActivo?.localidad].map(x => String(x || '').trim()).filter(Boolean).join(', ')
    // ── Datos base ──────────────────────────────────────────────────────────
    ...
    const rubrosHTML = Object.entries(rubrosAgrup)
      ...
      .map(([label, data]) => {
        const pct = totalGastosTotal > 0 ? (data.total/totalGastosTotal*100).toFixed(2).replace('.', ',') : '0,00'
    ...
    // ── U4: art. 11 inc. e — honorarios del administrador ──
    const honG = (gastos || []).filter(g => g.categoria === 'honorarios_admin')
    const boxSt = 'border:1px solid #ccc;border-radius:4px;padding:6px 10px;font-size:7.5pt;line-height:1.6;margin-top:8px'
    const avisoSt = 'border:1px dashed #d97706;background:#fffbeb;color:#92400e;border-radius:4px;padding:6px 10px;font-size:7.5pt;margin-top:8px'
    const honHTML = honG.length
      ? `<div style="${boxSt}"><div style="color:#1A3FA0;font-weight:700;font-size:8pt;margin-bottom:2px">HONORARIOS DE ADMINISTRACI&Oacute;N (art. 11 inc. e Ley 14.701)</div>${honG.map(g =>
          `<div><strong>Administrador:</strong> ${escH(admNombre)} &nbsp; <strong>CUIT:</strong> ${escH(admCuit)}${admFiscal ? ` &nbsp; <strong>Situaci&oacute;n fiscal:</strong> ${escH(admFiscal)}` : ''}<br/><strong>Comprobante:</strong> ${escH(g.comprobante || '—')}${g.fecha ? ' del ' + fDMY(g.fecha) : ''} &nbsp; <strong>Per&iacute;odo:</strong> ${per} &nbsp; <strong>Importe total:</strong> $ ${fmtN(parseFloat(g.monto) || 0)}</div>`).join('')}</div>`
      : (esPreliquidacion ? `<div style="${avisoSt}">&#9888; <strong>Preliquidaci&oacute;n:</strong> no hay gasto de honorarios de administraci&oacute;n cargado en este per&iacute;odo (art. 11 inc. e Ley 14.701).</div>` : '')

    // ── U4: art. 11 inc. j — seguro del consorcio (ficha del consorcio / póliza leída con IA) ──
    ...
    const cuotaPer = cuotasPol.filter(c => c && c.vencimiento && String(c.vencimiento).slice(0, 7) === String(expSel?.periodo || ''))
    const segHTML = (cia || consorcioActivo?.poliza_nro)
      ? `<div style="${boxSt}"><div style="color:#1A3FA0;font-weight:700;font-size:8pt;margin-bottom:2px">SEGURO DEL CONSORCIO (art. 11 inc. j Ley 14.701)</div>
          <div><strong>Compa&ntilde;&iacute;a:</strong> ${escH(cia || '—')} &nbsp; <strong>P&oacute;liza N&deg;:</strong> ${escH(consorcioActivo?.poliza_nro || '—')}${(consorcioActivo?.poliza_vto_desde || consorcioActivo?.poliza_vto_hasta) ? ` &nbsp; <strong>Vigencia:</strong> ${fDMY(consorcioActivo?.poliza_vto_desde)} al ${fDMY(consorcioActivo?.poliza_vto_hasta)}` : ''}${consorcioActivo?.poliza_suma ? ` &nbsp; <strong>Suma asegurada:</strong> $ ${fmtN(consorcioActivo.poliza_suma)}` : ''}</div>
          ${cobs.length ? `<div><strong>Coberturas:</strong> ${cobs.slice(0, 8).map(c => escH(c.cobertura) + (c.suma_asegurada != null ? ' $ ' + fmtN(c.suma_asegurada) : '')).join(' &middot; ')}</div>` : ''}
          ${cuotaPer.length ? `<div><strong>Cuota del per&iacute;odo:</strong> ${cuotaPer.map(c => `N&deg; ${escH(c.nro ?? '?')}${(cuotasPol.length && Number(c.nro) <= cuotasPol.length) ? ' de ' + cuotasPol.length : ''} &mdash; vto ${fDMY(c.vencimiento)}${c.importe != null ? ' &mdash; $ ' + fmtN(c.importe) : ''}`).join(' &middot; ')}</div>` : ''}
          <div style="color:#555">Los pagos de cuotas del per&iacute;odo se detallan en &quot;Pagos del per&iacute;odo&quot;.</div></div>`
      : (esPreliquidacion ? `<div style="${avisoSt}">&#9888; <strong>Preliquidaci&oacute;n:</strong> el consorcio no tiene datos de p&oacute;liza cargados en la Ficha (art. 11 inc. j Ley 14.701). Cargala o us&aacute; &laquo;Analizar con IA&raquo;.</div>` : '')
...
<div class="page">
    ...
    <div class="hdr-right">
      ...
      <div class="datos-row">
        <div class="datos-col">
          ...
          <p><strong>${escH(admNombre)}</strong><br/>
          Domicilio: ${escH(admDom)}<br/>
          ${escH(admEmail)}<br/>
          CUIT: ${escH(admCuit)} &nbsp; Inscripci&oacute;n RPAC: ${escH(admRpac)}${admFiscal ? ` &nbsp; ${escH(admFiscal)}` : ''}<br/>
          ${admTel ? `Tel: ${escH(admTel)}` : ''}</p>
        </div>
        <div class="datos-col">
          <h4>Consorcio</h4>
          <p><strong>${(consorcioActivo?.nombre||'').replace(/</g,'&lt;')}</strong><br/>
          ${domCon ? `${escH(domCon)}<br/>` : ''}CUIT: ${escH(cuitCon)}<br/>
          ${consorcioActivo?.clave_suterh ? `Clave SUTERH: ${escH(consorcioActivo.clave_suterh)}` : ''}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    ...
    <span>R.P.A.C.: ${escH(admRpac)} | CUIT: ${escH(cuitCon)} | Vto: ${fechaVto1}</span>
...
<div class="page">
  ...
  <div class="footer">
    <span>${(consorcioActivo?.nombre||'').replace(/</g,'&lt;')} &mdash; Liquidaci&oacute;n ${per}</span>
    <span>R.P.A.C.: ${escH(admRpac)}</span>
...
<div class="page">
  <div style="display:flex;justify-content:space-between;font-size:7pt;margin-bottom:4px">
    <div><strong>Administraci&oacute;n:</strong> ${escH(admNombre)} &nbsp; <strong>Consorcio:</strong> ${(consorcioActivo?.nombre||'').replace(/</g,'&lt;')} &nbsp; <strong>Per&iacute;odo:</strong> ${expSel?.periodo||''}</div>
    <div style="text-align:right"><strong>N&deg; RPA: ${escH(admRpac)}</strong> &nbsp; <strong>CUIT:</strong> ${escH(cuitCon)} &nbsp; <strong>Vencimiento:</strong> ${fechaVto1}</div>
  </div>
  ...
  <div class="footer">
    <span>${(consorcioActivo?.nombre||'').replace(/</g,'&lt;')} &mdash; Liquidaci&oacute;n ${per}</span>
    <span>R.P.A.C. N&deg;${escH(admRpac)} | CUIT: ${escH(cuitCon)}</span>
...
// U2 (2026-09-25): copia serializable de los datos del PDF para guardar al cierre (con_expensas.pdf_datos).
// Descarta textos muy largos (ej. imágenes en base64) que no usa la plantilla.
export function prepararDatosPDF(datos) {
  return JSON.parse(JSON.stringify(datos || {}, (k, v) => (typeof v === 'string' && v.length > 50000) ? undefined : v))
}

// Abre en una ventana nueva el PDF guardado al cierre. Devuelve false si el navegador bloqueó la ventana.
export function abrirLiquidacionGuardada(pdfDatos) {
  const w = window.open('', '_blank', 'width=1100,height=800,scrollbars=yes,resizable=yes')
  if (!w) return false
  escribirLiquidacionNativa(w, construirHTMLLiquidacionNativa({ ...pdfDatos, esPreliquidacion: false }))
  return true
}

// Escribe el HTML en una ventana ya abierta (abrirla en el gesto del usuario evita el bloqueo de pop-ups).
export function escribirLiquidacionNativa(printWin, html) {
  try {

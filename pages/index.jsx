...
const BUILD_VERSION = '20260524-asambleas-v2-v44'
...
// ══════════════════════════════════════════════════════════════════════════════
// ══════════════════════════════════════════════════════════════════════════════
// EQUIPO — Gestión de usuarios (multi-usuario)
// ══════════════════════════════════════════════════════════════════════════════
function Equipo({ session }) {
  ...
  async function revocar(id,email) {
...
// ══════════════════════════════════════════════════════════════════════════════
// ASAMBLEAS v2 — Componente unificado sin sub-componentes separados
// ══════════════════════════════════════════════════════════════════════════════
function Asambleas({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
  ...
  const EST_L = {convocatoria:'📋 Convocatoria',realizada:'✅ Realizada',acta_generada:'📄 Acta generada',acta_aprobada:'✔ Aprobada'}
  ...
  const NOTA  = 'Art. 2060 CCCN: para ser "decisión" deben estar presentes más del 50% de los consorcistas representando más del 50% de la propiedad (doble mayoría). Si no se alcanza, lo resuelto es "propuesta" que debe notificarse a ausentes (15 días para oposición). No existe primer ni segundo llamado en PH (Ley 14.701). El administrador debe documentar el resultado de cada punto (art. 2062 CCCN).'
  async function cargar() {
  ...
  function abrirDetalle(a) {
    setDetalle({...a}); setActaEdit(a.acta_borrador||a.acta_final||'')
    setTranscEdit(a.transcripcion||''); setPresentes((a.unidades_presentes||[]).join(', '))
    setHoraFin(a.hora_fin||''); setTabDet('convocatoria'); setVista('detalle')
  }
  function nuevoForm(tipo) {
    const od = tipo==='ordinaria'
      ? ['Declaración de la validez de la Asamblea.','Designación del Presidente de la Asamblea.','Designación del Secretario de Acta y dos copropietarios para firmarla.','Rendición de cuentas y estado financiero.','Renovación de mandato del administrador o cambio del mismo.','Designación de miembros del Consejo de Propietarios.']
      : ['Declaración de la validez de la Asamblea.','Designación del Presidente de la Asamblea.','Designación del Secretario de Acta y dos copropietarios para firmarla.']
    setForm({tipo,modalidad:'virtual',hora:'19:00',plataforma_virtual:'zoom',orden_del_dia:od,estado:'convocatoria',fecha:''})
    setVista('form')
  }
  async function guardarForm() {
    if (!form.fecha) return setMsg({tipo:'warn',texto:'La fecha es requerida'})
    const od = (form.orden_del_dia||[]).filter(p=>p.trim())
    const { error } = await supabase.from('con_asambleas').upsert([{...form,id:form.id||('ASM-'+consorcioId+'-'+Date.now()),admin_id:session.user.id,consorcio_id:consorcioId,orden_del_dia:od,updated_at:new Date().toISOString()}],{onConflict:'id'})
    if (error) return setMsg({tipo:'error',texto:error.message})
    setMsg({tipo:'ok',texto:'✓ Guardado'}); setVista('lista'); setForm(null); cargar()
  }
  function textoConv(a) {
    if (!a) return ''
    const cn=consorcioActivo?.nombre||'...'; const tipo=a.tipo==='ordinaria'?'ORDINARIA':'EXTRAORDINARIA'
    ...
    const cab='Del Lenguado 1313 Local 3 — Pinamar · (02254) 51-6386\nadministración@administracionpinamar.com\n\n'
    ...
    if (a.modalidad==='virtual'||a.modalidad==='mixta') {
      ...
      return cab+'CONVOCATORIA A ASAMBLEA GENERAL '+tipo+'\n'+cn.toUpperCase()+' — PINAMAR\n'+mixta+'\n— '+fC+' — '+a.hora+' hs —\n\nDe mi mayor consideración:\n\nEn carácter de administrador del '+cn+', lo invito a una Asamblea General '+tipoL+' a celebrarse bajo '+moda+' el día '+fL+' a las '+a.hora+' hs.\n\nORDEN DEL DÍA\n'+od+'\n\nIngreso — link de acceso:\n'+(a.link_virtual||'[COMPLETAR LINK]')+'\n'+clave+'\n\nEs requisito: nombre y apellido visibles; cámara encendida al acreditarse y votar.'+firma
    }
    return cab+'CONVOCATORIA A ASAMBLEA GENERAL '+tipo+'\n'+cn.toUpperCase()+' — PINAMAR\n'+(a.lugar?'A las '+a.hora+' horas en '+a.lugar+'\n':'\n')+'\nDe mi mayor consideración:\n\nPor la presente, y en mi carácter de Administrador del '+cn+', convoco a Asamblea General '+tipoL+', para el día '+fL+' a las '+a.hora+' horas en '+(a.lugar||'...')+'\n\nORDEN DEL DÍA\n'+od+'\n\nSin más por el particular, saluda atte.'+firma
  }
  async function enviarConv() {
    const cps=copropietarios.filter(c=>c.consorcio_id===consorcioId&&c.email)
    if (!cps.length) return setMsg({tipo:'warn',texto:'No hay propietarios con email en este consorcio'})
    if (!confirm('¿Enviar convocatoria a '+cps.length+' propietarios?')) return
    setEnviando(true); setMsg(null)
    const {data:{session:sess}}=await supabase.auth.getSession()
    const texto=textoConv(detalle); const tipoL=detalle.tipo==='ordinaria'?'Ordinaria':'Extraordinaria'
    const asunto='Convocatoria Asamblea General '+tipoL+' — '+(consorcioActivo?.nombre||'')+' — '+fmtF(detalle.fecha)
    let ok=0,err=0
    for (const cp of cps) {
      try { const r=await fetch(SUPA_URL+'/functions/v1/enviar-notificacion',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+sess?.access_token,'apikey':process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||''},body:JSON.stringify({destinatarios:[cp.email],asunto,cuerpo_texto:texto,consorcio_id:consorcioId})}); if(r.ok)ok++;else err++ } catch{err++}
    }
  ...
  async function guardarTranscripcion() {
    ...
    await supabase.from('con_asambleas').update({transcripcion:transcEdit,unidades_presentes:pres,total_presentes:pres.length,hora_fin:horaFin,estado:'realizada',updated_at:new Date().toISOString()}).eq('id',detalle.id)
  ...
  async function generarActa() {
    ...
    setGenerando(true); setMsg(null)
    const {data:{session:sess}}=await supabase.auth.getSession()
    const res=await fetch(SUPA_URL+'/functions/v1/generar-acta-asamblea',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+sess?.access_token,'apikey':process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||''},body:JSON.stringify({transcripcion:transcEdit,consorcio_nombre:consorcioActivo?.nombre,consorcio_direccion:consorcioActivo?.direccion,tipo_asamblea:detalle.tipo,modalidad:detalle.modalidad,plataforma_virtual:detalle.plataforma_virtual,fecha:fmtF(detalle.fecha),hora_inicio:detalle.hora,hora_fin:horaFin,orden_del_dia:detalle.orden_del_dia||[],unidades_presentes:detalle.unidades_presentes||[],total_ufs:unidades.length,pct_presentes:detalle.pct_presentes,matricula_rpi:consorcioActivo?.matricula_rpi,escritura_nro:consorcioActivo?.escritura_nro})})
    const data=await res.json()
    ...
    setTabDet('acta'); setMsg({tipo:'ok',texto:'✓ Acta generada — revisá y editá antes de aprobar'})
    setGenerando(false); cargar()
  }
  async function guardarActa(aprobar) {
    const upd=aprobar?{acta_final:actaEdit,estado:'acta_aprobada',updated_at:new Date().toISOString()}:{acta_borrador:actaEdit,updated_at:new Date().toISOString()}
    await supabase.from('con_asambleas').update(upd).eq('id',detalle.id)
    setDetalle(d=>({...d,...upd})); setMsg({tipo:'ok',texto:aprobar?'✔ Acta aprobada':'✓ Borrador guardado'}); cargar()
  }
    ...
    <div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
          ...
          {(form.modalidad==='presencial'||form.modalidad==='mixta')&&(<div style={{gridColumn:'span 3'}}><Input label="Lugar" value={form.lugar||''} onChange={v=>setForm(x=>({...x,lugar:v}))} placeholder="SUM del edificio..."/></div>)}
          {(form.modalidad==='virtual'||form.modalidad==='mixta')&&(<>
            <div><div style={{fontSize:12,color:GR,marginBottom:4}}>Plataforma</div>
              <select value={form.plataforma_virtual||'zoom'} onChange={e=>setForm(x=>({...x,plataforma_virtual:e.target.value}))} style={{width:'100%',padding:'8px',border:'1px solid #d1d5db',borderRadius:7,fontSize:13}}>{PLATS.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select>
            </div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        {(form.orden_del_dia||[]).map((p,i)=>(
          <div key={i} style={{display:'flex',gap:8,marginBottom:8,alignItems:'flex-start'}}>
            ...
            <textarea value={p} onChange={e=>{const od=[...(form.orden_del_dia||[])];od[i]=e.target.value;setForm(x=>({...x,orden_del_dia:od}))}} rows={2} style={{flex:1,padding:'8px',border:'1px solid #d1d5db',borderRadius:7,fontSize:12.5,fontFamily:'inherit',resize:'vertical'}}/>
            <button type="button" onClick={()=>setForm(x=>({...x,orden_del_dia:(x.orden_del_dia||[]).filter((_,j)=>j!==i)}))} style={{padding:'6px 10px',background:'#fee2e2',color:RJ,border:'none',borderRadius:6,cursor:'pointer',fontSize:12,marginTop:4}}>✕</button>
          </div>
        ))}
        <button type="button" onClick={()=>setForm(x=>({...x,orden_del_dia:[...(x.orden_del_dia||[]),'']}))} style={{padding:'7px 16px',background:'#eff6ff',color:AZ,border:'1px solid '+AZ,borderRadius:7,fontSize:12,fontWeight:600,cursor:'pointer',marginTop:4}}>+ Agregar punto</button>
      </Card>
  ...
  if (vista==='detalle'&&detalle) return (
    <div>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:14}}>
        ...
        <BtnSec onClick={()=>{setForm({...detalle,orden_del_dia:detalle.orden_del_dia||[]});setVista('form')}} style={{fontSize:12}}>✏ Editar</BtnSec>
      </div>
      <Msg data={msg}/>
      <div style={{display:'flex',gap:0,marginBottom:16,borderBottom:'2px solid #e5e7eb',overflowX:'auto'}}>
        {[['convocatoria','📋 Convocatoria'],['asistencia','👥 Asistencia'],['transcripcion','🎙 Transcripción'],['acta','📄 Acta']].map(([id,l])=>(
          <button key={id} type="button" onClick={()=>setTabDet(id)} style={{padding:'8px 14px',border:'none',borderBottom:tabDet===id?'2px solid '+AZ:'2px solid transparent',background:'transparent',color:tabDet===id?AZ:GR,fontWeight:tabDet===id?700:400,fontSize:12.5,cursor:'pointer',marginBottom:-2,whiteSpace:'nowrap'}}>{l}</button>
        ))}
      </div>
        ...
        <div>
          <Card style={{marginBottom:14}}>
            ...
            {(detalle.orden_del_dia||[]).map((p,i)=>(
              <div key={i} style={{display:'flex',gap:8,marginBottom:6,fontSize:13}}><span style={{color:GR,fontWeight:600,minWidth:20}}>{i+1}.</span><span>{p}</span></div>
            ))}
          </Card>
          ...
          <Card style={{marginBottom:14}}>
            ...
            <textarea readOnly value={textoConv(detalle)} rows={22} style={{width:'100%',padding:'12px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:12,fontFamily:'Georgia, serif',lineHeight:1.8,background:'#fafafa',resize:'vertical',boxSizing:'border-box'}}/>
          </Card>
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            <Btn onClick={()=>{navigator.clipboard.writeText(textoConv(detalle));setMsg({tipo:'ok',texto:'✓ Texto copiado'})}}>📋 Copiar texto</Btn>
            <Btn onClick={enviarConv} disabled={enviando} style={{opacity:enviando?0.5:1,background:'#16a34a'}}>{enviando?'⏳ Enviando...':'✉ Enviar a propietarios'+(detalle.convocatoria_enviada?' (re-enviar)':'')}</Btn>
          </div>
          {detalle.convocatoria_enviada&&<div style={{marginTop:10,fontSize:12,color:VD}}>✓ Convocatoria ya enviada</div>}
        </div>
      )}
      ...
      {tabDet==='asistencia'&&(
        <Card style={{marginBottom:14}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
            <div>
              <div style={{fontSize:12,color:GR,marginBottom:4}}>Unidades presentes (separadas por coma)</div>
              <textarea value={presentes} onChange={e=>setPresentes(e.target.value)} rows={6} placeholder="1A, 2B, 3C, PB-E..." style={{width:'100%',padding:'10px',border:'1px solid #d1d5db',borderRadius:7,fontSize:12.5,fontFamily:'inherit',resize:'vertical',boxSizing:'border-box'}}/>
              <div style={{fontSize:11,color:GR,marginTop:4}}>{presentes.split(',').filter(p=>p.trim()).length} unidades</div>
            </div>
            <div style={{background:'#f8fafc',borderRadius:8,padding:'12px',fontSize:13}}>
              ...
              <div style={{marginBottom:10}}>Porcentaje: <strong style={{color:presentes.split(',').filter(p=>p.trim()).length/Math.max(unidades.length,1)>0.5?VD:RJ}}>{unidades.length>0?((presentes.split(',').filter(p=>p.trim()).length/unidades.length)*100).toFixed(1):0}%</strong></div>
              <div style={{fontSize:12,color:GR,marginBottom:4}}>Hora de finalización</div>
              <input type="time" value={horaFin} onChange={e=>setHoraFin(e.target.value)} style={{padding:'6px',border:'1px solid #d1d5db',borderRadius:6,fontSize:13,width:'100%'}}/>
            </div>
          </div>
      ...
      {tabDet==='transcripcion'&&(
        <div>
          <div style={{fontSize:12,color:GR,marginBottom:10}}>Pegá la transcripción completa (de Zoom, Meet, o manual). La IA generará el acta formal según arts. 2060-2067 CCCN y Ley 14.701.</div>
          <Card style={{marginBottom:14}}>
            <textarea value={transcEdit} onChange={e=>setTranscEdit(e.target.value)} rows={22} placeholder="Transcripción completa de la asamblea..." style={{width:'100%',padding:'12px',border:'1px solid #d1d5db',borderRadius:8,fontSize:12.5,fontFamily:'inherit',lineHeight:1.7,resize:'vertical',boxSizing:'border-box'}}/>
            <div style={{display:'flex',gap:8,marginTop:10}}>
              <Btn onClick={guardarTranscripcion}>💾 Guardar transcripción</Btn>
              <Btn onClick={generarActa} disabled={generando||!transcEdit.trim()} style={{background:'#7c3aed',opacity:generando||!transcEdit.trim()?0.5:1}}>{generando?'⏳ Generando acta...':'🤖 Generar acta con IA'}</Btn>
            </div>
          </Card>
        </div>
      )}
        ...
        <div>
          {!actaEdit&&!detalle.acta_borrador?(
            <Card><div style={{textAlign:'center',padding:'24px 0',color:GR}}>
              ...
              <div style={{fontSize:12,marginBottom:16}}>Cargá la transcripción y generá el acta con IA</div>
              <BtnSec onClick={()=>setTabDet('transcripcion')}>Ir a Transcripción →</BtnSec>
            </div></Card>
          ):(
            <div>
              <div style={{fontSize:12,color:GR,marginBottom:10}}>{detalle.acta_final?'✔ Acta aprobada':'📄 Borrador — revisá y editá antes de aprobar'}</div>
              <textarea value={actaEdit} onChange={e=>setActaEdit(e.target.value)} rows={32} style={{width:'100%',padding:'14px',border:'1px solid '+(detalle.acta_final?VD:'#d1d5db'),borderRadius:8,fontSize:13,fontFamily:'Georgia, serif',lineHeight:1.9,resize:'vertical',boxSizing:'border-box',background:detalle.acta_final?'#f0fdf4':'#fff'}}/>
              <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
                <Btn onClick={()=>guardarActa(false)}>💾 Guardar borrador</Btn>
                {!detalle.acta_final&&<Btn onClick={()=>{if(confirm('¿Aprobar el acta definitivamente?'))guardarActa(true)}} style={{background:VD}}>✔ Aprobar acta</Btn>}
                <Btn onClick={()=>{navigator.clipboard.writeText(actaEdit);setMsg({tipo:'ok',texto:'✓ Acta copiada'})}} style={{background:'#374151'}}>📋 Copiar</Btn>
                <Btn onClick={generarActa} disabled={generando} style={{background:'#7c3aed',opacity:generando?0.5:1}}>{generando?'⏳ Regenerando...':'🤖 Regenerar con IA'}</Btn>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div>
      ...
      <div style={{fontSize:12,color:GR,marginBottom:16}}>Convocatorias, actas y seguimiento de asambleas</div>
      <Msg data={msg}/>
      {asambleas.length===0?(
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {asambleas.map(a=>{
            ...
            return (
              <Card key={a.id} onClick={()=>abrirDetalle(a)} style={{cursor:'pointer',border:esP?'1.5px solid '+AZ:'1px solid #e5e7eb',background:esP?'#f8fbff':'#fff'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                      <span style={{fontWeight:700,fontSize:13}}>{'Asamblea General '+(a.tipo==='ordinaria'?'Ordinaria':'Extraordinaria')}</span>
                      <span style={{fontSize:11,padding:'2px 8px',borderRadius:6,fontWeight:600,background:a.estado==='acta_aprobada'?'#dcfce7':a.estado==='acta_generada'?'#ede9fe':a.estado==='realizada'?'#fff8e1':'#eff6ff',color:EST_C[a.estado]}}>{EST_L[a.estado]}</span>
                    </div>
                    <div style={{fontSize:12,color:GR}}>{fmtF(a.fecha)} — {a.hora} hs ·{a.modalidad==='virtual'?' Virtual ('+(a.plataforma_virtual||'Zoom')+')':a.modalidad==='mixta'?' Presencial + Virtual':' Presencial'}{a.lugar?' · '+a.lugar:''}</div>
                    <div style={{fontSize:11,color:GR,marginTop:4}}>{(a.orden_del_dia?.length||0)+' puntos en Orden del Día'}{a.convocatoria_enviada?' · ✉ Enviada':''}{a.acta_final?' · ✔ Acta aprobada':a.acta_borrador?' · 📄 Borrador':''}</div>
                  </div>

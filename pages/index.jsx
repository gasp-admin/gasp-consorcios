...
const BUILD_VERSION = '20260524-asambleas-v3-v46'
...
// ══════════════════════════════════════════════════════════════════════════════
// ASAMBLEAS v3 — Todas las funciones como const arrow para evitar SWC hoisting
// ══════════════════════════════════════════════════════════════════════════════
function Asambleas({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
  ...
  const NOTA = 'Nota del Director del RPAC (art. 2060 CCCN): para ser "decisión" deben estar presentes más del 50% de los consorcistas representando más del 50% de la propiedad (doble mayoría). Si no se alcanza, lo resuelto es "propuesta" sujeta a notificación a ausentes con plazo de 15 días para oposición. No existe primer ni segundo llamado en PH (Ley 14.701). El administrador debe documentar el resultado de cada punto (art. 2062 CCCN).'
    ...
    for (const cp of cps) {
      try {
        const r = await fetch(SUPA_URL+'/functions/v1/enviar-notificacion', {
          ...
          body: JSON.stringify({ destinatarios:[cp.email], asunto, cuerpo_texto:convTexto, consorcio_id:consorcioId })
        })
        if (r.ok) ok++; else err++
      } catch { err++ }
    }
  ...
  const guardarTranscripcion = async () => {
    ...
    await supabase.from('con_asambleas').update({
      ...
      unidades_presentes: pres,
      total_presentes: pres.length,
      hora_fin: horaFin,
      estado: 'realizada',
      updated_at: new Date().toISOString()
    }).eq('id', detalle.id)
  ...
  const generarActa = async () => {
    ...
    const res = await fetch(SUPA_URL+'/functions/v1/generar-acta-asamblea', {
      ...
      body: JSON.stringify({
        ...
        consorcio_nombre: consorcioActivo?.nombre,
        consorcio_direccion: consorcioActivo?.direccion,
        tipo_asamblea: detalle.tipo,
        modalidad: detalle.modalidad,
        plataforma_virtual: detalle.plataforma_virtual,
        fecha: fmtF(detalle.fecha),
        hora_inicio: detalle.hora,
        hora_fin: horaFin,
        orden_del_dia: detalle.orden_del_dia || [],
        unidades_presentes: detalle.unidades_presentes || [],
    ...
    if (!res.ok || data.error) {
      setMsg({ tipo:'error', texto:'Error: '+(data.error||'desconocido') })
      setGenerando(false)
      return
    }
    ...
    setTabDet('acta')
    setMsg({ tipo:'ok', texto:'✓ Acta generada — revisá y editá antes de aprobar' })
    setGenerando(false)
    cargar()
  }
  ...
  const guardarActa = async (aprobar) => {
    const upd = aprobar
      ? { acta_final:actaEdit, estado:'acta_aprobada', updated_at:new Date().toISOString() }
  ...
  if (vista === 'form' && form) return (
    <div>
      <div style={{ fontWeight:700, fontSize:15, marginBottom:16 }}>
        {form.id ? '✏ Editar' : '+ Nueva Asamblea '+(form.tipo==='ordinaria'?'Ordinaria':'Extraordinaria')}
      </div>
      <Msg data={msg} />
      <Card style={{ marginBottom:14 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
          ...
          {(form.modalidad==='presencial'||form.modalidad==='mixta') && (
            <div style={{ gridColumn:'span 3' }}>
              <Input label="Lugar presencial" value={form.lugar||''} onChange={v=>setForm(x=>({...x,lugar:v}))} placeholder="SUM del edificio, calle..." />
            </div>
          )}
      ...
      <Card style={{ marginBottom:14 }}>
        ...
        {(form.orden_del_dia||[]).map((p,i) => (
          <div key={i} style={{ display:'flex', gap:8, marginBottom:8, alignItems:'flex-start' }}>
            ...
            <textarea value={p}
              onChange={e => { const od=[...(form.orden_del_dia||[])]; od[i]=e.target.value; setForm(x=>({...x,orden_del_dia:od})) }}
              rows={2} style={{ flex:1, padding:'8px', border:'1px solid #d1d5db', borderRadius:7, fontSize:12.5, fontFamily:'inherit', resize:'vertical' }} />
            <button type="button"
              onClick={() => setForm(x => ({...x, orden_del_dia:(x.orden_del_dia||[]).filter((_,j)=>j!==i)}))}
        ...
        <button type="button"
          onClick={() => setForm(x => ({...x, orden_del_dia:[...(x.orden_del_dia||[]),'']}))}
      ...
      <div style={{ display:'flex', gap:8 }}>
        <Btn onClick={guardarForm}>💾 Guardar</Btn>
        <BtnSec onClick={() => { setVista('lista'); setForm(null) }}>Cancelar</BtnSec>
      </div>
    </div>
  )
  ...
  if (vista === 'detalle' && detalle) return (
    <div>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontWeight:700, fontSize:14 }}>
            {'Asamblea General '+(detalle.tipo==='ordinaria'?'Ordinaria':'Extraordinaria')}
          </div>
          <div style={{ fontSize:12, color:GR }}>
            {fmtF(detalle.fecha)} — {detalle.hora} hs ·{' '}
            <span style={{ fontWeight:600, color:EST_C[detalle.estado] }}>{EST_L[detalle.estado]}</span>
          </div>
        </div>
        <BtnSec onClick={() => { setForm({...detalle, orden_del_dia:detalle.orden_del_dia||[]}); setVista('form') }}
          style={{ fontSize:12 }}>✏ Editar</BtnSec>
      </div>
      <Msg data={msg} />
      {/* Tabs navegación */}
        ...
        {[['convocatoria','📋 Convocatoria'],['asistencia','👥 Asistencia'],['transcripcion','🎙 Transcripción'],['acta','📄 Acta']].map(([id,l]) => (
          <button key={id} type="button" onClick={() => setTabDet(id)}
            style={{ padding:'8px 14px', border:'none',
              borderBottom: tabDet===id ? '2px solid '+AZ : '2px solid transparent',
              background:'transparent', color:tabDet===id?AZ:GR,
              fontWeight:tabDet===id?700:400, fontSize:12.5, cursor:'pointer', marginBottom:-2, whiteSpace:'nowrap' }}>
            {l}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          {(detalle.modalidad==='virtual'||detalle.modalidad==='mixta') && detalle.link_virtual && (
            <Card style={{ marginBottom:14, background:'#eff6ff', border:'1px solid #bfdbfe' }}>
              ...
              <a href={detalle.link_virtual} target="_blank" rel="noreferrer"
                style={{ color:AZ, fontWeight:600, fontSize:13 }}>{detalle.link_virtual}</a>
              {detalle.clave_virtual && (
                <div style={{ fontSize:12, color:GR, marginTop:4 }}>Clave: {detalle.clave_virtual}</div>
              )}
            </Card>
          )}
          <Card style={{ marginBottom:14 }}>
            <div style={{ fontWeight:600, color:AZ, fontSize:13, marginBottom:8 }}>
              Texto de la convocatoria
              <span style={{ fontWeight:400, color:GR, fontSize:11, marginLeft:8 }}>(editable — modificá antes de enviar)</span>
            </div>
            <textarea value={convTexto} onChange={e => setConvTexto(e.target.value)} rows={24}
              style={{ width:'100%', padding:'12px', border:'1px solid #e5e7eb', borderRadius:8,
                fontSize:12, fontFamily:'Georgia, serif', lineHeight:1.8, background:'#fafafa',
                resize:'vertical', boxSizing:'border-box' }} />
          </Card>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            <Btn onClick={() => { const t=generarTexto(detalle); setConvTexto(t); setMsg({tipo:'ok',texto:'✓ Texto regenerado desde datos'}) }}
              style={{ background:'#374151' }}>
              🔄 Regenerar texto
            </Btn>
            <Btn onClick={() => { navigator.clipboard.writeText(convTexto); setMsg({tipo:'ok',texto:'✓ Texto copiado al portapapeles'}) }}>
              📋 Copiar texto
            </Btn>
            <Btn onClick={enviarConv} disabled={enviando}
              style={{ opacity:enviando?0.5:1, background:'#16a34a' }}>
              {enviando ? '⏳ Enviando...' : '✉ Enviar a propietarios'+(detalle.convocatoria_enviada?' (re-enviar)':'')}
            </Btn>
          </div>
          {detalle.convocatoria_enviada && (
            <div style={{ marginTop:10, fontSize:12, color:VD }}>✓ Convocatoria ya enviada anteriormente</div>
          )}
        </div>
      )}
        ...
        <Card style={{ marginBottom:14 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
            <div>
              ...
              <textarea value={presentes} onChange={e => setPresentes(e.target.value)} rows={6}
                placeholder="Ej: 1A, 2B, 3C, PB-E, 5F TII..."
                style={{ width:'100%', padding:'10px', border:'1px solid #d1d5db', borderRadius:7,
                  fontSize:12.5, fontFamily:'inherit', resize:'vertical', boxSizing:'border-box' }} />
              <div style={{ fontSize:11, color:GR, marginTop:4 }}>
                {presentes.split(',').filter(p=>p.trim()).length} unidades ingresadas
              </div>
            </div>
            <div style={{ background:'#f8fafc', borderRadius:8, padding:'14px', fontSize:13 }}>
              ...
              <div style={{ marginBottom:6 }}>
                Presentes: <strong>{presentes.split(',').filter(p=>p.trim()).length}</strong>
              </div>
              <div style={{ marginBottom:12 }}>
                Porcentaje: <strong style={{ color: presentes.split(',').filter(p=>p.trim()).length/Math.max(unidades.length,1) > 0.5 ? VD : RJ }}>
                  {unidades.length > 0 ? ((presentes.split(',').filter(p=>p.trim()).length/unidades.length)*100).toFixed(1) : 0}%
                </strong>
                <span style={{ fontSize:11, color:GR, marginLeft:6 }}>(doble mayoría requiere {'>'} 50%)</span>
              </div>
              <div style={{ fontSize:12, color:GR, marginBottom:4 }}>Hora de finalización</div>
              <input type="time" value={horaFin} onChange={e => setHoraFin(e.target.value)}
                style={{ padding:'6px', border:'1px solid #d1d5db', borderRadius:6, fontSize:13, width:'100%' }} />
            </div>
          </div>
      ...
      {tabDet === 'transcripcion' && (
        <div>
          <div style={{ fontSize:12, color:GR, marginBottom:10 }}>
            Pegá la transcripción completa de la asamblea (exportada de Zoom, Meet, o manual).
            La IA generará el acta formal según arts. 2060-2067 CCCN y Ley 14.701.
          </div>
          <Card style={{ marginBottom:14 }}>
            <textarea value={transcEdit} onChange={e => setTranscEdit(e.target.value)} rows={22}
              ...
              style={{ width:'100%', padding:'12px', border:'1px solid #d1d5db', borderRadius:8,
                fontSize:12.5, fontFamily:'inherit', lineHeight:1.7, resize:'vertical', boxSizing:'border-box' }} />
            <div style={{ display:'flex', gap:8, marginTop:10 }}>
              <Btn onClick={guardarTranscripcion}>💾 Guardar transcripción</Btn>
              <Btn onClick={generarActa} disabled={generando || !transcEdit.trim()}
                style={{ background:'#7c3aed', opacity: generando||!transcEdit.trim() ? 0.5 : 1 }}>
      ...
      {tabDet === 'acta' && (
        <div>
          {!actaEdit && !detalle.acta_borrador ? (
            <Card>
              <div style={{ textAlign:'center', padding:'24px 0', color:GR }}>
                ...
                <div style={{ fontSize:12, marginBottom:16 }}>
                  Primero cargá la transcripción y generá el acta con IA
                </div>
            ...
            <div>
              <div style={{ fontSize:12, color:GR, marginBottom:10 }}>
                {detalle.acta_final ? '✔ Acta aprobada' : '📄 Borrador — revisá y editá antes de aprobar'}
              </div>
              <textarea value={actaEdit} onChange={e => setActaEdit(e.target.value)} rows={32}
                style={{ width:'100%', padding:'14px',
                  border:'1px solid '+(detalle.acta_final?VD:'#d1d5db'), borderRadius:8,
                  fontSize:13, fontFamily:'Georgia, serif', lineHeight:1.9, resize:'vertical',
                  boxSizing:'border-box', background:detalle.acta_final?'#f0fdf4':'#fff' }} />
              <div style={{ display:'flex', gap:8, marginTop:10, flexWrap:'wrap' }}>
                <Btn onClick={() => guardarActa(false)}>💾 Guardar borrador</Btn>
                {!detalle.acta_final && (
                  <Btn onClick={() => { if(confirm('¿Aprobar el acta definitivamente?')) guardarActa(true) }}
                    style={{ background:VD }}>
                    ✔ Aprobar acta
                  </Btn>
                )}
                <Btn onClick={() => { navigator.clipboard.writeText(actaEdit); setMsg({tipo:'ok',texto:'✓ Acta copiada'}) }}
                  style={{ background:'#374151' }}>
                  📋 Copiar
                </Btn>
                <Btn onClick={generarActa} disabled={generando}
                  style={{ background:'#7c3aed', opacity:generando?0.5:1 }}>
  ...
  return (
    <div>
      ...
      {asambleas.length === 0 ? (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {asambleas.map(a => {
            const dias = diasR(a.fecha)
            const esProx = a.estado === 'convocatoria' && dias !== null && dias >= 0
            return (
              <Card key={a.id} onClick={() => abrirDetalle(a)}
                style={{ cursor:'pointer',
                  border: esProx ? '1.5px solid '+AZ : '1px solid #e5e7eb',
                  background: esProx ? '#f8fbff' : '#fff' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                      <span style={{ fontWeight:700, fontSize:13 }}>
                        {'Asamblea General '+(a.tipo==='ordinaria'?'Ordinaria':'Extraordinaria')}
                      </span>
                      <span style={{ fontSize:11, padding:'2px 8px', borderRadius:6, fontWeight:600,
                        background: a.estado==='acta_aprobada'?'#dcfce7':a.estado==='acta_generada'?'#ede9fe':a.estado==='realizada'?'#fff8e1':'#eff6ff',
                        color: EST_C[a.estado] }}>
                        {EST_L[a.estado]}
                      </span>
                    </div>
                    <div style={{ fontSize:12, color:GR }}>

...
const BUILD_VERSION = '20260517-interes-tabla-pdf-consorcios-v2'
...
<div class="page">
  ...
  function handlePDF() {
    exportarPDF({
      titulo: 'Listado de Consorcios',
      logoB64: LOGO_ADM_B64,
      columnas: [
        { key:'nombre',   label:'Nombre', align:'left' },
        { key:'cuit',     label:'CUIT', align:'left' },
        { key:'domicilio',label:'Domicilio', align:'left' },
        { key:'banco',    label:'Banco', align:'left' },
        { key:'cbu',      label:'CBU', align:'left' },
        { key:'alias',    label:'Alias', align:'left' },
        { key:'suterh',   label:'Clave SUTERH', align:'left' },
        { key:'comision', label:'Com. %', align:'right' },
      ],
      filas: filtrados.map(c => ({
        nombre:   c.nombre || '',
        cuit:     c.cuit || '',
        ...
        cbu:      c.cbu || '',
        alias:    c.alias_cbu || '',
        suterh:   c.clave_suterh || '',
        comision: c.comision_pct ? c.comision_pct + '%' : '',
      })),
      totales: { nombre: `${filtrados.length} consorcio${filtrados.length!==1?'s':''}`, cuit:'',domicilio:'',banco:'',cbu:'',alias:'',suterh:'',comision:'' }
    })
  }
  ...
  function handleExcel() {
    exportarExcel({
      ...
      filas: filtrados.map(c => ({
        nombre: c.nombre||'', cuit: c.cuit||'', domicilio: c.domicilio||'',
        banco: c.banco||'', cbu: c.cbu||'', alias: c.alias_cbu||'',
        nro_cta: c.nro_cuenta||'', suterh: c.clave_suterh||'',
        comision: c.comision_pct||'', mora: c.interes_mora||'',
      }))
    })
  }
  ...
  return (
    <div>
      ...
      <BarraListado
        busqueda={busqueda} onBuscar={setBusqueda}
        onPDF={handlePDF} onExcel={handleExcel}
        placeholder="Buscar por nombre, CUIT, domicilio, banco..." />
      ...
      {filtrados.length === 0 ? (
        ...
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr style={{ background:'#f3f4f6' }}>
                {['Nombre','CUIT','Domicilio','Banco','CBU / Alias','SUTERH','Com.%','Mora%'].map((h,i) => (
                  <th key={i} style={{ padding:'8px 12px', textAlign:'left', fontSize:11,
                    fontWeight:700, color:GR, textTransform:'uppercase', borderBottom:'1px solid #e5e7eb',
                    whiteSpace:'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtrados.map((c,i) => (
                <tr key={c.id} style={{ borderBottom:'1px solid #f3f4f6', background: i%2===0?'#fff':'#fafafa' }}>
                  <td style={{ padding:'10px 12px', fontWeight:700, color:AZ }}>{c.nombre}</td>
                  <td style={{ padding:'10px 12px', fontSize:12, color:GR }}>{c.cuit||'—'}</td>
                  <td style={{ padding:'10px 12px', fontSize:12 }}>{c.domicilio||'—'}</td>
                  <td style={{ padding:'10px 12px', fontSize:12 }}>{c.banco||'—'}</td>
                  <td style={{ padding:'10px 12px', fontSize:11 }}>
                    {c.cbu && <div style={{ fontFamily:'monospace' }}>{c.cbu}</div>}
                    {c.alias_cbu && <div style={{ color:AZ, fontSize:10 }}>{c.alias_cbu}</div>}
                    {!c.cbu && !c.alias_cbu && '—'}
                  </td>
                  <td style={{ padding:'10px 12px', fontSize:12, fontWeight:c.clave_suterh?600:400, color:c.clave_suterh?'#374151':GR }}>
                    {c.clave_suterh||'—'}
                  </td>
                  <td style={{ padding:'10px 12px', fontSize:12, textAlign:'right' }}>
                    {c.comision_pct ? c.comision_pct + '%' : '—'}
                  </td>
                  <td style={{ padding:'10px 12px', fontSize:12, textAlign:'right',
                    fontWeight: parseFloat(c.interes_mora||0) > 0 ? 700 : 400,
                    color: parseFloat(c.interes_mora||0) > 0 ? AM : GR }}>
            ...
            <tfoot>
              <tr style={{ background:'#f0f4ff', borderTop:'2px solid '+AZ }}>
                <td colSpan={8} style={{ padding:'8px 12px', fontWeight:700, color:AZ, fontSize:12 }}>
                  Total: {filtrados.length} consorcio{filtrados.length!==1?'s':''} administrado{filtrados.length!==1?'s':''}
                </td>
              </tr>
            </tfoot>
...
export default function App() {
 ...
 const NAV=[
    ...
    { id:'listado_consorcios', label:'Mis Consorcios',     icon:'🏛️', sec:'Inicio' },

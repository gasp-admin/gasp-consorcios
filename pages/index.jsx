...
const BUILD_VERSION = '20260521-cert-ia-drive-v35'
...
function CertificadoLibreDeuda({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
  ...
  async function leerDesdeDrive(tipo = 'ambos') {
    if (!consorcioActivo?.drive_folder_id) {
      return setMsg({ tipo:'warn', texto:'Este consorcio no tiene carpeta Drive configurada. Configurar en Mis Consorcios → Editar.' })
    }
    setLeyendoDrive(true); setMsg(null); setResultadoDrive(null)
    try {
      // Obtener token Google del usuario vía la sesión de Supabase
      // El token de Google viene de la sesión OAuth de Google Drive MCP
      // Como alternativa, llamamos a la EF que recibe el folder_id
      // y usa la API de Drive con el service account o token del usuario
      const { data: { session: sess } } = await supabase.auth.getSession()

      // Intentar obtener el google token desde window.gapi o el provider token
      const googleToken = sess?.provider_token || ''
      if (!googleToken) {
        setMsg({ tipo:'warn', texto:'⚠ Para leer desde Drive automáticamente, la sesión debe tener acceso a Google. Si el botón falla, completar los datos manualmente.' })
        setLeyendoDrive(false)
        return
      }

      const res = await fetch(`${SUPA_URL}/functions/v1/extraer-datos-consorcio-drive`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${sess?.access_token}`,
      ...
      // Aplicar datos del Reglamento
      if (data.reglamento?.datos) {
        const d = data.reglamento.datos
        if (d.matricula_rpi)     { setMatriculaRPI(d.matricula_rpi);    hayDatos = true }
        if (d.escritura_nro)     { setEscrituraNro(d.escritura_nro);    hayDatos = true }
        if (d.escritura_fecha)   { setEscrituraFecha(d.escritura_fecha); hayDatos = true }
        if (d.escribano_nombre)  { setEscribanoReg(d.escribano_nombre); hayDatos = true }
        mensajes.push(`✓ Reglamento: "${data.reglamento.archivo}"`)
      } else if (data.error_reglamento) {
        mensajes.push(`⚠ Reglamento: ${data.error_reglamento}`)
      }

      // Aplicar datos de la Póliza
      if (data.poliza?.datos) {
        const d = data.poliza.datos
        if (d.poliza_nro)        { setPolizaNro(d.poliza_nro);          hayDatos = true }
        if (d.poliza_compania)   { setPolizaComp(d.poliza_compania);    hayDatos = true }
        if (d.poliza_vto_desde)  { setPolizaDesde(d.poliza_vto_desde);  hayDatos = true }
        if (d.poliza_vto_hasta)  { setPolizaHasta(d.poliza_vto_hasta);  hayDatos = true }
        if (d.poliza_suma)       { setPolizaSuma(String(d.poliza_suma)); hayDatos = true }
        mensajes.push(`✓ Póliza: "${data.poliza.archivo}"`)
      } else if (data.error_poliza) {
        mensajes.push(`⚠ Póliza: ${data.error_poliza}`)
      }

      setResultadoDrive({ reglamento: data.reglamento, poliza: data.poliza,
        archivos: data.archivos_encontrados || [], mensajes })

      if (hayDatos) {
        setMsg({ tipo:'ok', texto:`🤖 IA completó los campos automáticamente. ${mensajes.join(' · ')}` })
      } else {
        setMsg({ tipo:'warn', texto:`IA ejecutada pero sin datos extraíbles. ${mensajes.join(' · ')} — Completar manualmente.` })
      }
    } catch(e: any) {
      setMsg({ tipo:'warn', texto:`No se pudo leer desde Drive automáticamente: ${e.message}. Completar los datos manualmente.` })
    }
    setLeyendoDrive(false)
  }
        ...
        <div>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap' }}>
            ...
            {consorcioActivo?.drive_folder_id && (
              <button type="button" disabled={leyendoDrive}
                ...
                {leyendoDrive ? '⏳ Leyendo Drive...' : '🤖 Leer Reglamento y Póliza con IA'}
              </button>
            )}
          </div>
          ...
          {resultadoDrive && (
            <div style={{ background:'#f5f3ff', border:'1px solid #ddd6fe', borderRadius:10, padding:'12px 16px', marginBottom:14, fontSize:12 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                {resultadoDrive.reglamento && (
                  <div style={{ background:'#fff', borderRadius:8, padding:'8px 12px' }}>
                    <div style={{ fontWeight:600, color:'#374151', marginBottom:4 }}>📖 Reglamento</div>
                    <div style={{ color:GR, fontSize:11 }}>{resultadoDrive.reglamento.archivo}</div>
                    {resultadoDrive.reglamento.datos && (
                      <div style={{ marginTop:6, fontSize:11, color:'#374151', lineHeight:1.7 }}>
                        {resultadoDrive.reglamento.datos.matricula_rpi && <div>Matrícula RPI: <strong>{resultadoDrive.reglamento.datos.matricula_rpi}</strong></div>}
                        {resultadoDrive.reglamento.datos.escritura_nro && <div>Escritura N°: <strong>{resultadoDrive.reglamento.datos.escritura_nro}</strong></div>}
                        {resultadoDrive.reglamento.datos.escribano_nombre && <div>Escribano/a: <strong>{resultadoDrive.reglamento.datos.escribano_nombre}</strong></div>}
                      </div>
                    )}
                  </div>
                )}
                {resultadoDrive.poliza && (
                  <div style={{ background:'#fff', borderRadius:8, padding:'8px 12px' }}>
                    <div style={{ fontWeight:600, color:'#374151', marginBottom:4 }}>🛡️ Póliza</div>
                    <div style={{ color:GR, fontSize:11 }}>{resultadoDrive.poliza.archivo}</div>
                    {resultadoDrive.poliza.datos && (
                      <div style={{ marginTop:6, fontSize:11, color:'#374151', lineHeight:1.7 }}>
                        {resultadoDrive.poliza.datos.poliza_nro && <div>N°: <strong>{resultadoDrive.poliza.datos.poliza_nro}</strong></div>}
                        {resultadoDrive.poliza.datos.poliza_compania && <div>Cía: <strong>{resultadoDrive.poliza.datos.poliza_compania}</strong></div>}
                        {resultadoDrive.poliza.datos.poliza_vto_hasta && <div>Hasta: <strong>{resultadoDrive.poliza.datos.poliza_vto_hasta}</strong></div>}
                      </div>
                    )}
                  </div>
                )}
              </div>
              {resultadoDrive.archivos?.length > 0 && (
                <div style={{ marginTop:8, fontSize:10.5, color:GR }}>
                  Archivos en carpeta: {resultadoDrive.archivos.map((a: any) => a.nombre).join(' · ')}
                </div>
              )}
            </div>
          )}

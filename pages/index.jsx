...
const BUILD_VERSION = '20260524-FINAL-v49'
...
// ══════════════════════════════════════════════════════════════════════════════
// ASAMBLEAS — Convocatorias, Actas IA, Actas Subidas, Mandatos, Calendario (v5)
// ══════════════════════════════════════════════════════════════════════════════
function Asambleas({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
  ...
  const [pdfSubido,    setPdfSubido]    = useState(null)      // { url, name }
  const [analizandoIA, setAnalizandoIA] = useState(false)
  const [iaResultado,  setIaResultado]  = useState(null)      // { consorcio, tipo, mandato, fecha_inicio, fecha_fin, resumen }
  // Mandato
  const [mandatoForm,     setMandatoForm]     = useState(null)   // { fecha_inicio, fecha_fin, duracion_anos, observaciones }
  ...
  // Helper: nombre del consorcio por ID (para la lista multi-consorcio)
  const consorcioNombre = (cid) => {
    if (!cid) return ''
    if (consorcioActivo?.id === cid) return consorcioActivo?.nombre || ''
    // Si viene de otro consorcio, mostrar el ID resumido como fallback
    return cid.replace('CON','Consorcio ')
  }

  // ── HELPERS CALENDARIO ────────────────────────────────────────────────────
  const crearAlertaCalendario = async ({ consorcioId, descripcion, fecha_vencimiento, fecha_aviso1, fecha_aviso2, tipo, notas }) => {
    const id = 'AGV-'+consorcioId+'-'+tipo+'-'+Date.now()
    const { data, error } = await supabase.from('con_agenda_vencimientos').insert([{
      id, admin_id: session.user.id, consorcio_id: consorcioId,
      tipo, descripcion, fecha_vencimiento, fecha_aviso1, fecha_aviso2,
      estado: 'pendiente', notas, created_at: new Date().toISOString()
    }]).select().single()
    if (error) console.error('Alerta calendario:', error)
    return data?.id || null
  }
  ...
  const cargar = async () => {
    // Sin filtro de consorcio: mostrar todas las asambleas del admin
  ...
  const abrirDetalle = (a) => {
    ...
    setPdfSubido(a.acta_subida_pdf_url ? { url:a.acta_subida_pdf_url, name:'Acta subida' } : null)
    setIaResultado(null)
  ...
  const guardarForm = async () => {
    ...
    if (error) return setMsg({ tipo:'error', texto:error.message })
    // Crear alerta en calendario para la convocatoria (solo al crear, no al editar)
    if (isNueva && form.fecha) {
      const alertaId = await crearAlertaCalendario({
        ...
        notas: 'Convocatoria cargada automáticamente desde módulo Asambleas. Recordar preparar documentación.'
      })
      if (alertaId) {
        await supabase.from('con_asambleas').update({ calendario_convocatoria_id: alertaId }).eq('id', newId)
      }
    }
  ...
  const generarActa = async () => {
    ...
    const res = await fetch(SUPA_URL+'/functions/v1/generar-acta-asamblea', {
      ...
      body: JSON.stringify({
        ...
        total_ufs:unidades.length, matricula_rpi:consorcioActivo?.matricula_rpi, escritura_nro:consorcioActivo?.escritura_nro
      })
    })
    const data = await res.json()
  ...
  const subirPDF = async (file) => {
    if (!file || file.type !== 'application/pdf') return setMsg({ tipo:'warn', texto:'Solo se admiten archivos PDF' })
    setSubiendoPDF(true); setMsg(null)
    try {
      const path = `asambleas/${session.user.id}/${consorcioId}/${detalle.id}_${Date.now()}.pdf`
      ...
      setDetalle(d => ({...d, acta_subida_pdf_url:url, acta_subida_pdf_path:path, acta_fuente:'subida'}))
      setPdfSubido({ url, name: file.name })
      setMsg({ tipo:'ok', texto:'✓ PDF subido correctamente. Podés analizarlo con IA.' })
    } catch(e) {
      setMsg({ tipo:'error', texto:'Error al subir: '+e.message })
    }
  ...
  const analizarConIA = async () => {
    const pdfUrl = pdfSubido?.url || detalle.acta_subida_pdf_url
    if (!pdfUrl) return setMsg({ tipo:'warn', texto:'Primero subí un PDF' })
    setAnalizandoIA(true); setMsg(null)
    try {
      const { data:{ session:sess } } = await supabase.auth.getSession()
...
No incluyas texto fuera del JSON.`
      ...
      // Parsear respuesta — puede venir como texto o como objeto
      let parsed = data
      if (typeof data.resultado === 'string') {
        try { parsed = JSON.parse(data.resultado) } catch { parsed = data }
      ...
      await supabase.from('con_asambleas').update({
        ia_procesada: true, ia_resumen: parsed.resumen || '',
        updated_at: new Date().toISOString()
      }).eq('id', detalle.id)
      setDetalle(d => ({...d, ia_procesada:true, ia_resumen:parsed.resumen||''}))
      // Si hay mandato, pre-rellenar el form de mandato
      if (parsed.mandato_administrador) {
        setMandatoForm({
          fecha_inicio:  parsed.mandato_fecha_inicio  || '',
          fecha_fin:     parsed.mandato_fecha_fin     || '',
          duracion_anos: parsed.mandato_duracion_anos || 1,
          observaciones: parsed.mandato_tipo ? ('IA detectó: '+parsed.mandato_tipo) : ''
        })
      }
      setMsg({ tipo:'ok', texto:'✓ Análisis completado. Revisá los datos detectados.' })
    } catch(e) {
      setMsg({ tipo:'error', texto:'Error en IA: '+e.message })
    }
    setAnalizandoIA(false)
  }

  // ── GUARDAR MANDATO + ALERTA CALENDARIO ──────────────────────────────────
  const guardarMandato = async () => {
    if (!mandatoForm?.fecha_inicio || !mandatoForm?.fecha_fin)
      return setMsg({ tipo:'warn', texto:'Ingresá fecha de inicio y fin del mandato' })
    setGuardandoMandato(true); setMsg(null)
    // Crear alerta de vencimiento de mandato en calendario
    const alertaId = await crearAlertaCalendario({
      consorcioId,
      descripcion: 'Vencimiento mandato administrador — '+(consorcioActivo?.nombre||''),
      fecha_vencimiento: mandatoForm.fecha_fin,
      fecha_aviso1: agregarDias(mandatoForm.fecha_fin, -90),  // 3 meses antes
      fecha_aviso2: agregarDias(mandatoForm.fecha_fin, -30),  // 1 mes antes
      tipo: 'mandato_administrador',
      notas: 'Preparar convocatoria a Asamblea para renovación/designación de administrador.\n'+
             'Inicio mandato: '+mandatoForm.fecha_inicio+' — Fin: '+mandatoForm.fecha_fin+
             (mandatoForm.observaciones ? '\n'+mandatoForm.observaciones : '')
    })
    await supabase.from('con_asambleas').update({
      mandato_administrador:  true,
      mandato_fecha_inicio:   mandatoForm.fecha_inicio,
      mandato_fecha_fin:      mandatoForm.fecha_fin,
      mandato_duracion_anos:  mandatoForm.duracion_anos,
      mandato_observaciones:  mandatoForm.observaciones,
      calendario_mandato_id:  alertaId,
      updated_at: new Date().toISOString()
    }).eq('id', detalle.id)
    setDetalle(d => ({...d,
      mandato_administrador: true,
    ...
    setMsg({ tipo:'ok', texto:'✓ Mandato guardado — Alertas en calendario: 90 y 30 días antes del vencimiento' })
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
          ...
          <div>
            ...
            <select value={form.modalidad} onChange={e=>setForm(x=>({...x,modalidad:e.target.value}))}
              style={{width:'100%',padding:'8px',border:'1px solid #d1d5db',borderRadius:7,fontSize:13}}>
              {[['presencial','Presencial'],['virtual','Virtual (Zoom/Meet)'],['mixta','Presencial + Virtual']].map(([v,l])=><option key={v} value={v}>{l}</option>)}
            </select>
          </div>
          {(form.modalidad==='presencial'||form.modalidad==='mixta')&&(<div style={{gridColumn:'span 3'}}><Input label="Lugar" value={form.lugar||''} onChange={v=>setForm(x=>({...x,lugar:v}))} placeholder="Salón del edificio / dirección"/></div>)}
  ...
  if (vista === 'detalle' && detalle) return (
    <div>
      ...
      <div style={{display:'flex',gap:0,marginBottom:16,borderBottom:'2px solid #e5e7eb',overflowX:'auto'}}>
        {[['convocatoria','📋 Convocatoria'],['asistencia','👥 Asistencia'],['transcripcion','🎙 Transcripción'],
          ['acta','📄 Acta'],['actas_pdf','📎 Actas PDF'],['mandato','🔖 Mandato']].map(([id,l])=>(
          <button key={id} type="button" onClick={()=>setTabDet(id)}
            style={{padding:'8px 14px',border:'none',borderBottom:tabDet===id?'2px solid '+AZ:'2px solid transparent',
              background:'transparent',color:tabDet===id?AZ:GR,fontWeight:tabDet===id?700:400,cursor:'pointer',fontSize:12.5,whiteSpace:'nowrap'}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tabDet==='convocatoria'&&(<div>
        ...
        <Card style={{marginBottom:14}}>
          <div style={{fontWeight:600,color:AZ,fontSize:13,marginBottom:8}}>Texto de la convocatoria <span style={{fontWeight:400,color:GR,fontSize:11}}>(editable)</span></div>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <Btn onClick={()=>{setConvTxt(generarTexto(detalle));setMsg({tipo:'ok',texto:'✓ Texto regenerado'})}}>🔄 Regenerar</Btn>
        ...
        {detalle.convocatoria_enviada&&<div style={{marginTop:10,fontSize:12,color:VD}}>✓ Convocatoria ya enviada el {fmtF(detalle.fecha_envio_conv)}</div>}
        {detalle.calendario_convocatoria_id&&(
          <div style={{marginTop:10,padding:'8px 12px',background:'#f0fdf4',borderRadius:8,fontSize:12,color:VD,border:'1px solid #bbf7d0'}}>
            📅 Alerta en calendario activa (avisos 15 y 7 días antes de la asamblea)
          </div>
        )}
      </div>)}
        ...
        <Card style={{marginBottom:14}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
            <div>
              ...
              <div style={{fontSize:11,color:GR,marginTop:4}}>{presentes.split(',').filter(p=>p.trim()).length} unidades ingresadas</div>
            </div>
            <div style={{background:'#f8fafc',borderRadius:8,padding:'14px',fontSize:13}}>
              ...
              <div style={{marginBottom:12}}>Porcentaje: <strong style={{color:presentes.split(',').filter(p=>p.trim()).length/unidades.length>=0.5?VD:RJ}}>{unidades.length>0?((presentes.split(',').filter(p=>p.trim()).length/unidades.length)*100).toFixed(1):0}%</strong></div>
      ...
      {tabDet==='transcripcion'&&(<div>
        <div style={{fontSize:12,color:GR,marginBottom:10}}>Pegá la transcripción completa (de Zoom, Meet, o manual)</div>
        <Card style={{marginBottom:14}}>
          <textarea value={transcEdit} onChange={e=>setTranscEdit(e.target.value)} rows={22} placeholder="Transcripción de la asamblea..."
          ...
          <div style={{display:'flex',gap:8,marginTop:10}}>
            ...
            <Btn onClick={generarActa} disabled={generando||!transcEdit.trim()} style={{background:'#7c3aed',opacity:generando||!transcEdit.trim()?0.5:1}}>
              {generando?'⏳ Generando acta...':'🤖 Generar acta con IA'}
            </Btn>
          </div>
        </Card>
      </div>)}

      {/* ── TAB ACTA (generada por IA desde transcripción) ── */}
      {tabDet==='acta'&&(<div>
        {!actaEdit&&!detalle.acta_borrador?(
          ...
          <div>
            <div style={{fontSize:12,color:GR,marginBottom:10}}>{detalle.acta_final?'✔ Acta aprobada':'📄 Borrador — revisá antes de aprobar'}</div>
            ...
            <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
              ...
              <Btn onClick={generarActa} disabled={generando} style={{background:'#7c3aed',opacity:generando?0.5:1}}>
                {generando?'⏳...':'🤖 Re-generar'}
              </Btn>
            </div>
          </div>
        )}
      ...
      {tabDet==='actas_pdf'&&(<div>
        <Card style={{marginBottom:14}}>
          ...
          <div style={{fontSize:12,color:GR,marginBottom:14}}>
            Subí un acta ya confeccionada (desde PC o Drive). La IA identificará automáticamente el consorcio, tipo de asamblea y mandatos.
          </div>
          {/* Zona de subida */}
          {!pdfSubido ? (
            <label style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
              ...
              onDragOver={e=>{e.preventDefault()}}
              onDrop={e=>{e.preventDefault();const f=e.dataTransfer.files[0];if(f)subirPDF(f)}}>
              <input type="file" accept="application/pdf" style={{display:'none'}}
                onChange={e=>e.target.files[0]&&subirPDF(e.target.files[0])}/>
              {subiendoPDF ? (
                <div style={{color:AZ,fontWeight:600}}>⏳ Subiendo PDF...</div>
              ) : (
                <>
                  <div style={{fontSize:36,marginBottom:8}}>📄</div>
                  <div style={{fontWeight:600,color:AZ,fontSize:13,marginBottom:4}}>Arrastrá el PDF aquí</div>
                  <div style={{fontSize:12,color:GR}}>o hacé click para seleccionar desde tu PC</div>
                </>
              )}
            </label>
          ) : (
            <div style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',background:'#f0fdf4',borderRadius:10,border:'1px solid #bbf7d0',marginBottom:14}}>
              ...
              <div style={{display:'flex',gap:8}}>
                <Btn onClick={analizarConIA} disabled={analizandoIA} style={{background:'#7c3aed',opacity:analizandoIA?0.5:1,fontSize:12}}>
                  {analizandoIA?'⏳ Analizando...':'🤖 Analizar con IA'}
                </Btn>
                <BtnSec onClick={()=>setPdfSubido(null)} style={{fontSize:12}}>Cambiar</BtnSec>
              </div>
            </div>
          )}
          ...
          {iaResultado && (
            <div style={{marginTop:14,padding:'16px',background:'#faf5ff',border:'1px solid #e9d5ff',borderRadius:10}}>
              <div style={{fontWeight:700,color:'#7c3aed',fontSize:13,marginBottom:10}}>🤖 Resultado del análisis IA</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,fontSize:13,marginBottom:12}}>
                ...
                <div><span style={{color:GR}}>Mandato admin:</span> <strong style={{color:iaResultado.mandato_administrador?VD:GR}}>{iaResultado.mandato_administrador?'✓ Detectado':'No detectado'}</strong></div>
                {iaResultado.mandato_tipo&&<div><span style={{color:GR}}>Tipo mandato:</span> <strong style={{textTransform:'capitalize'}}>{iaResultado.mandato_tipo}</strong></div>}
              ...
              {iaResultado.resoluciones_clave?.length>0&&(
                <div style={{marginBottom:10}}>
                  <div style={{fontSize:12,color:GR,marginBottom:4}}>Resoluciones clave:</div>
              ...
              {iaResultado.mandato_administrador&&(
                <div style={{marginTop:12,padding:'10px 12px',background:'#fef9c3',borderRadius:8,fontSize:12,color:'#92400e'}}>
                  ⚠️ Se detectó designación/renovación de mandato. Ir al tab <strong>🔖 Mandato</strong> para confirmar las fechas y activar el aviso de calendario.
                </div>
              )}
            </div>
          )}

          {/* Acta generada dentro del sistema — link a PDF si existe */}
          {(detalle.acta_pdf_url||detalle.acta_final)&&(
            <div style={{marginTop:14,padding:'12px 16px',background:'#eff6ff',borderRadius:10,border:'1px solid #bfdbfe'}}>
              <div style={{fontWeight:600,color:AZ,fontSize:13,marginBottom:6}}>📄 Acta generada en el sistema</div>
              {detalle.acta_pdf_url?<a href={detalle.acta_pdf_url} target="_blank" rel="noreferrer" style={{fontSize:13,color:AZ,fontWeight:600}}>Ver PDF del acta ↗</a>
      ...
      {tabDet==='mandato'&&(<div>
        <Card style={{marginBottom:14}}>
          ...
          <div style={{fontSize:12,color:GR,marginBottom:16}}>
            Registrá la designación o renovación del mandato. Se crearán automáticamente alertas en el calendario 90 y 30 días antes del vencimiento.
          </div>
          {detalle.mandato_administrador&&!mandatoForm&&(
            <div style={{padding:'12px 16px',background:'#f0fdf4',borderRadius:10,border:'1px solid #bbf7d0',marginBottom:14}}>
              ...
              {detalle.calendario_mandato_id&&(
                <div style={{marginTop:10,fontSize:12,color:VD}}>📅 Alertas activas: 90 y 30 días antes del {fmtF(detalle.mandato_fecha_fin)}</div>
              )}
              <BtnSec onClick={()=>setMandatoForm({
                fecha_inicio: detalle.mandato_fecha_inicio||'',
                fecha_fin:    detalle.mandato_fecha_fin||'',
                duracion_anos: detalle.mandato_duracion_anos||1,
                observaciones: detalle.mandato_observaciones||''
              })} style={{marginTop:10,fontSize:12}}>✏ Modificar</BtnSec>
            </div>
          )}
          {(!detalle.mandato_administrador||mandatoForm)&&(
            <div>
              {!mandatoForm&&(
                <div style={{textAlign:'center',padding:'20px 0',color:GR}}>
                  <div style={{fontSize:32,marginBottom:8}}>🔖</div>
                  <div style={{fontSize:13,marginBottom:16}}>Esta asamblea no tiene mandato registrado</div>
                ...
                <div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginBottom:12}}>
                    <div>
                      ...
                      <input type="date" value={mandatoForm.fecha_inicio}
                        onChange={e=>{
                          const fi = e.target.value
                          // Auto-calcular fecha fin según duración
                          let ff = mandatoForm.fecha_fin
                          if (fi && mandatoForm.duracion_anos) {
                            const d = new Date(fi+'T12:00:00')
                            d.setFullYear(d.getFullYear() + parseInt(mandatoForm.duracion_anos))
                            ff = d.toISOString().slice(0,10)
                          }
                          setMandatoForm(x=>({...x, fecha_inicio:fi, fecha_fin:ff}))
                        }}
                        style={{width:'100%',padding:'8px',border:'1px solid #d1d5db',borderRadius:7,fontSize:13}}/>
                    </div>
                    <div>
                      <div style={{fontSize:12,color:GR,marginBottom:4}}>Duración (años)</div>
                      <select value={mandatoForm.duracion_anos}
                        onChange={e=>{
                          const dur = parseInt(e.target.value)
                          let ff = mandatoForm.fecha_fin
                          if (mandatoForm.fecha_inicio && dur) {
                            const d = new Date(mandatoForm.fecha_inicio+'T12:00:00')
                            d.setFullYear(d.getFullYear() + dur)
                            ff = d.toISOString().slice(0,10)
                          }
                          setMandatoForm(x=>({...x, duracion_anos:dur, fecha_fin:ff}))
                        }}
                        style={{width:'100%',padding:'8px',border:'1px solid #d1d5db',borderRadius:7,fontSize:13}}>
                    ...
                    <div>
                      ...
                      <input type="date" value={mandatoForm.fecha_fin}
                        onChange={e=>setMandatoForm(x=>({...x,fecha_fin:e.target.value}))}
                        style={{width:'100%',padding:'8px',border:'1px solid #d1d5db',borderRadius:7,fontSize:13}}/>
                    </div>
                  </div>
                  <div style={{marginBottom:12}}>
                    <div style={{fontSize:12,color:GR,marginBottom:4}}>Observaciones</div>
                    <input value={mandatoForm.observaciones}
                      onChange={e=>setMandatoForm(x=>({...x,observaciones:e.target.value}))}
                      placeholder="ej: Designación, unanimidad, art. 2066 CCCN"
                      style={{width:'100%',padding:'8px',border:'1px solid #d1d5db',borderRadius:7,fontSize:13}}/>
                  </div>
                  {mandatoForm.fecha_fin&&(
                    <div style={{padding:'10px 14px',background:'#fffbeb',borderRadius:8,fontSize:12,color:'#92400e',marginBottom:12,border:'1px solid #fde68a'}}>
                      📅 Se crearán alertas en <strong>Agenda de Vencimientos</strong>:<br/>
                      • Aviso 1: {fmtF(agregarDias(mandatoForm.fecha_fin,-90))} (90 días antes)<br/>
                      • Aviso 2: {fmtF(agregarDias(mandatoForm.fecha_fin,-30))} (30 días antes)<br/>
                      • Vencimiento: {fmtF(mandatoForm.fecha_fin)}
                    </div>
                  )}
                  <div style={{display:'flex',gap:8}}>
                    <Btn onClick={guardarMandato} disabled={guardandoMandato} style={{opacity:guardandoMandato?0.5:1}}>
                      {guardandoMandato?'⏳ Guardando...':'🔖 Guardar y activar alertas'}
                    </Btn>
                    <BtnSec onClick={()=>setMandatoForm(null)}>Cancelar</BtnSec>
                  </div>
                </div>
              )}
            </div>
              ...
              <Card key={a.id} onClick={()=>abrirDetalle(a)}
                ...
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:11,color:GR,marginTop:4}}>
                      {(a.orden_del_dia?.length||0)+' puntos en Orden del Día'}
                    ...
                    {a.consorcio_id&&a.consorcio_id!==consorcioId&&(
                      <div style={{fontSize:10,color:AZ,marginTop:3,fontWeight:600,opacity:0.7}}>
...
// ════════════════════════════════════════════════════════════════════════════
// EMAIL TRACKING — Seguimiento de liquidaciones enviadas

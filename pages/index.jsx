...
// Genera y abre un HTML imprimible como PDF desde una tabla de datos
function exportarPDF({ titulo, subtitulo, columnas, filas, totales, consorcioNombre, logoB64 }) {
...
// Exporta los datos como archivo Excel/CSV descargable
function exportarExcel({ titulo, columnas, filas }) {
  const BOM = '\uFEFF'
  ...
  a.href = url; a.download = `${titulo.replace(/\s/g,'-')}-${new Date().toISOString().split('T')[0]}.csv`
  a.click()
  setTimeout(()=>URL.revokeObjectURL(url), 5000)
}

// Componente de barra de búsqueda + botones de exportación reutilizable
function BarraListado({ busqueda, onBuscar, onPDF, onExcel, placeholder = 'Buscar...' }) {
  return (
    <div style={{ display:'flex', gap:8, marginBottom:12, flexWrap:'wrap', alignItems:'center' }}>
      <div style={{ flex:1, minWidth:200, position:'relative' }}>
        <span style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', color:GR, fontSize:14 }}>🔍</span>
        <input value={busqueda} onChange={e=>onBuscar(e.target.value)}
...
function Copropietarios({ session, consorcioId, onUpdate }) {
  ...
  function handlePDF() {
    exportarPDF({titulo:'Listado de Copropietarios',logoB64:LOGO_ADM_B64,
      columnas:[{key:'nombre',label:'Apellido y Nombre'},{key:'dni',label:'DNI'},
        {key:'tel',label:'Teléfono'},{key:'email',label:'Email'},{key:'cons',label:'Consejero'}],
      filas:filtrados.map(cp=>({nombre:cp.apellido_nombre||'',dni:cp.dni||'',
        tel:cp.telefono||'',email:cp.email||'',cons:cp.es_consejero?'Sí':''}))
    })
  }
  function handleExcel() {
    exportarExcel({titulo:'Copropietarios',
      columnas:[{key:'nombre',label:'Apellido y Nombre'},{key:'dni',label:'DNI'},{key:'tel',label:'Teléfono'},
        {key:'email',label:'Email'},{key:'cuit',label:'CUIT/CUIL'},{key:'cons',label:'Consejero'}],
      filas:filtrados.map(cp=>({nombre:cp.apellido_nombre||'',dni:cp.dni||'',tel:cp.telefono||'',
        email:cp.email||'',cuit:cp.cuit_cuil||'',cons:cp.es_consejero?'Sí':''}))
    })
  }
...
function Proveedores({ session, consorcioId }) {
  ...
  function handlePDF() {
    exportarPDF({titulo:'Listado de Proveedores',logoB64:LOGO_ADM_B64,
...
// ══════════════════════════════════════════════════════════════════════════════
// COBRANZAS AUTOMÁTICAS — Importar pagos con pantalla de revisión UF por UF
// Versión V2 — 20/05/2026 — Pantalla de revisión basada en imagen de referencia
// Soporta: SIRO · Expensas Pagas · Banco Macro/Roela · Galicia · Provincia A/B
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
// ENVIAR NOTIFICACIÓN — Comunicación libre a UFs del consorcio
// Permite adjuntar archivos y/o link de carpeta Drive del consorcio
// ══════════════════════════════════════════════════════════════════════════════
function EnviarNotificacion({ session, consorcioId, consorcioActivo, unidades, copropietarios, adminPerfil }) {
...
// ══════════════════════════════════════════════════════════════════════════════
// CONSULTAR ENVIADOS — Historial completo de emails del consorcio + lectura
// ══════════════════════════════════════════════════════════════════════════════
function ConsultarEnviados({ session, consorcioId, unidades, copropietarios }) {
...
// ══════════════════════════════════════════════════════════════════════════════
// RECLAMOS — Sistema de tickets de copropietarios
// ══════════════════════════════════════════════════════════════════════════════
function Reclamos({ session, consorcioId, unidades, copropietarios }) {
...
// ══════════════════════════════════════════════════════════════════════════════
// RENDICIÓN DE CUENTAS — Informe mensual automático
// ══════════════════════════════════════════════════════════════════════════════
function RendicionCuentas({ session, consorcioId, consorcioActivo, expensas, copropietarios, unidades }) {
...
// ══════════════════════════════════════════════════════════════════════════════
// AGENDA DE VENCIMIENTOS — Recordatorios de pólizas, contratos, impuestos
// ══════════════════════════════════════════════════════════════════════════════
function AgendaVencimientos({ session, consorcioId, consorcioActivo, proveedores }) {
  ...
  const TIPOS = [
    ...
    ['mantenimiento','🔨 Mantenimiento'],['impuesto','📋 Impuesto/tasa'],['reunion','👥 Reunión/Asamblea'],['otro','📌 Otro'],
  ]
  const hoy = new Date().toISOString().split('T')[0]
    ...
    <div>
      ...
      <Card>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
          ...
          <Input label="Fecha de vencimiento" value={form.fecha_vencimiento||''} onChange={v=>setForm(x=>({...x,fecha_vencimiento:v}))} type="date" required />
          <div style={{ gridColumn:'span 2' }}>
            <Input label="Descripción" value={form.descripcion||''} onChange={v=>setForm(x=>({...x,descripcion:v}))} required placeholder="Ej: Renovación póliza Allianz — ATICA II" />
          </div>
          <Input label="Monto ($)" value={form.monto||''} onChange={v=>setForm(x=>({...x,monto:v}))} type="number" />
          <Input label="Aviso anticipado (días)" value={form.fecha_aviso1||''} onChange={v=>setForm(x=>({...x,fecha_aviso1:v}))} type="date" placeholder="Fecha del primer aviso" />
          <div style={{ gridColumn:'span 2' }}>
  ...
  return (
    <div>
      ...
      <div style={{ fontSize:12, color:GR, marginBottom:16 }}>Pólizas, contratos, mantenimientos e impuestos</div>
      <Msg data={msg} />

      <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap' }}>
        {[['todos','📋 Todos'],['proximos','⏰ Próximos 30 días'],['urgentes','🚨 Esta semana'],['polizas','🛡 Pólizas']].map(([v,l])=>(
      ...
      {filtrados.length === 0 ? (
        <Card>
          <div style={{ textAlign:'center', padding:'24px 0', color:GR }}>
            <div style={{ fontSize:36, marginBottom:8 }}>📅</div>
            {filtro==='proximos'?'Sin vencimientos en los próximos 30 días':'Sin vencimientos en esta categoría'}
          </div>
        </Card>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {filtrados.map(v => {
            ...
            return (
              <Card key={v.id} style={{ border: dias<0?`1.5px solid ${RJ}`:dias<=7?`1.5px solid ${AM}`:'1px solid #e5e7eb' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:600, fontSize:13 }}>{v.descripcion}</div>
                    <div style={{ fontSize:11, color:GR, marginTop:2 }}>
                      {TIPOS.find(([tv])=>tv===v.tipo)?.[1]||v.tipo} · Vence: {fmtF(v.fecha_vencimiento)}
                  ...
                  <div style={{ textAlign:'right', marginLeft:14 }}>
                    ...
                    <button type="button" onClick={()=>completar(v.id)}
                      style={{ marginTop:4, padding:'3px 10px', background:VD, color:'#fff', border:'none', borderRadius:6, fontSize:11, fontWeight:600, cursor:'pointer' }}>
                      ✓ Completado
                    </button>
                  </div>
                </div>
...
// ══════════════════════════════════════════════════════════════════════════════
// CERTIFICADO LIBRE DEUDA — Art. 2067 inc. l) CCCN
// Genera el certificado completando datos del consorcio, expensas y póliza
// Copia el modelo en Drive y guarda registro en BD
// ══════════════════════════════════════════════════════════════════════════════
function CertificadoLibreDeuda({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
...
// ══════════════════════════════════════════════════════════════════════════════
// ASAMBLEAS — Gestión completa: Convocatoria, Acta IA, Registro
// ══════════════════════════════════════════════════════════════════════════════
function Asambleas({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
  ...
  const hoy = new Date().toISOString().split('T')[0]
  const PLATAFORMAS = [['zoom','Zoom'],['meet','Google Meet'],['teams','Microsoft Teams'],['jitsi','Jitsi Meet'],['otro','Otra plataforma']]
  const ESTADOS_LABEL = { convocatoria:'📋 Convocatoria', realizada:'✅ Realizada', acta_generada:'📄 Acta generada', acta_aprobada:'✔ Acta aprobada' }
  const ESTADOS_COLOR = { convocatoria:AZ, realizada:AM, acta_generada:'#7c3aed', acta_aprobada:VD }

  // Puntos de orden del día por defecto según tipo
  const PUNTOS_DEFAULT_ORD = [
  ...
  // ── Nuevo form ──────────────────────────────────────────────────────────────
  function nuevoForm(tipo) {
    const puntos = tipo === 'ordinaria' ? [...PUNTOS_DEFAULT_ORD] : [...PUNTOS_DEFAULT_EXT]
    setForm({
      tipo, modalidad:'virtual', hora:'19:00', plataforma_virtual:'zoom',
      orden_del_dia: puntos, estado:'convocatoria',
      fecha: ''
    })
    setTab('form')
  }
  ...
  async function guardar() {
    ...
    if (!form.hora)  return setMsg({ tipo:'warn', texto:'La hora es requerida' })
    const od = (form.orden_del_dia||[]).filter(p => p.trim())
    const payload = {
      ...form,
      id: form.id || `ASM-${consorcioId}-${Date.now()}`,
      admin_id: session.user.id,
      consorcio_id: consorcioId,
      orden_del_dia: od,
      updated_at: new Date().toISOString(),
    }
    delete payload.convocatoria_html  // se genera por separado
    const { error } = await supabase.from('con_asambleas').upsert([payload], { onConflict:'id' })
    if (error) return setMsg({ tipo:'error', texto: error.message })
    setMsg({ tipo:'ok', texto:'✓ Asamblea guardada' })
    setForm(null); setTab('lista'); cargar()
  }

  // ── Generar texto de convocatoria ───────────────────────────────────────────
  function generarTextoConvocatoria(asm) {
    const cn = consorcioActivo?.nombre || '...'
    const cd = consorcioActivo?.direccion || '...'
    const tipo = asm.tipo === 'ordinaria' ? 'ORDINARIA' : 'EXTRAORDINARIA'
    const fechaStr = asm.fecha ? new Date(asm.fecha+'T12:00:00').toLocaleDateString('es-AR',{day:'2-digit',month:'2-digit',year:'numeric'}) : '...'
    const moda = asm.modalidad === 'virtual' ? 'MODALIDAD A DISTANCIA' : asm.modalidad === 'mixta' ? 'MODALIDAD PRESENCIAL Y VIRTUAL' : 'MODALIDAD PRESENCIAL'
    const lugarStr = asm.modalidad === 'virtual' ? '' : `\nA las ${asm.hora} horas en ${asm.lugar || '...'}\n`
    const plat = asm.plataforma_virtual ? (asm.plataforma_virtual.charAt(0).toUpperCase() + asm.plataforma_virtual.slice(1)) : 'Zoom'
    const od = (asm.orden_del_dia||[]).map((p,i) => `${i+1}. ${p}`).join('\n')

    const notaRPAC = `Nota del Director del RPAC, con aclaraciones sobre la legislación vigente referida a las Asambleas:\n\nDe acuerdo a lo normado en el art. 2060 del Código Civil y Comercial, para ser considerada una "decisión" deben encontrarse presentes más del 50% de la totalidad de los consorcistas que representen más del 50% de la propiedad del consorcio (doble mayoría). Si no alcanzara ese número, lo que se resuelva se denominará "propuesta" y deberá ser notificado a todos los propietarios ausentes para que realicen su oposición dentro de los 15 días corridos. No existe primer ni segundo llamado en Propiedad Horizontal. El administrador debe realizar una nota que exprese cómo quedó la votación de cada punto (art. 2062 CCCN).`

    if (asm.modalidad === 'virtual' || asm.modalidad === 'mixta') {
      const lineaMixta = asm.modalidad === 'mixta' ? '\nPresencial: ' + (asm.lugar || '...') + ' — y a distancia vía ' + plat : ''
      const lineaClave = asm.clave_virtual ? 'Clave: ' + asm.clave_virtual : ''
      const fechaLarga = asm.fecha ? new Date(asm.fecha+'T12:00:00').toLocaleDateString('es-AR',{day:'numeric',month:'long',year:'numeric'}) : '...'
      const tipoLabel  = asm.tipo === 'ordinaria' ? 'Ordinaria' : 'Extraordinaria'
      return 'Del Lenguado 1313 Local 3 — Pinamar · (02254) 51-6386 / (02267) 15 444035\n' +
        'administración@administracionpinamar.com · www.administracionpinamar.com\n\n' +
        'CONVOCATORIA A ASAMBLEA GENERAL ' + tipo + '\n' +
        cn.toUpperCase() + ' — PINAMAR\n' +
        lineaMixta + '\n' +
        '— ' + fechaStr + ' — ' + asm.hora + ' hs — ' + moda + ' —\n\n' +
        'De mi mayor consideración:\n\n' +
        'En carácter de administrador del ' + cn + ', lo invito a una Asamblea General ' + tipoLabel + ' a celebrarse bajo ' + moda.toLowerCase() + ' el día ' + fechaLarga + ' a las ' + asm.hora + ' hs.\n\n' +
        'ORDEN DEL DÍA\n' + od + '\n\n' +
        'Ingreso, reglas para acreditarse y deliberar:\n\n' +
        'Se ingresará a la hora indicada al siguiente link:\n' +
        (asm.link_virtual || '[COMPLETAR LINK]') + '\n' +
        lineaClave + '\n\n' +
        'La reunión será coordinada por el administrador. Es requisito tener identificado nombre, apellido y departamento, cámara encendida al momento de acreditarse y al momento de votar.\n\n' +
        'Javier García Pérez — Administrador — Mat. RPAC N° 83\n\n' + notaRPAC
    } else {
      const fechaLarga2 = asm.fecha ? new Date(asm.fecha+'T12:00:00').toLocaleDateString('es-AR',{day:'numeric',month:'long',year:'numeric'}) : '...'
      const tipoLabel2  = asm.tipo === 'ordinaria' ? 'Ordinaria' : 'Extraordinaria'
      return 'Del Lenguado 1313 Local 3 — Pinamar · (02254) 51-6386 / (02267) 15 444035\n' +
        'administración@administracionpinamar.com · www.administracionpinamar.com\n\n' +
        'CONVOCATORIA A ASAMBLEA GENERAL ' + tipo + '\n' +
        cn.toUpperCase() + ' — PINAMAR\n' +
        lugarStr + '\n' +
        'De mi mayor consideración:\n\n' +
        'Por la presente, y en mi carácter de Administrador del ' + cn + ', convoco a Asamblea General ' + tipoLabel2 + ', para el día ' + fechaLarga2 + ' a las ' + asm.hora + ' horas en ' + (asm.lugar || '...') + '\n\n' +
        'ORDEN DEL DÍA\n' + od + '\n\n' +
        'Sin más por el particular, saluda atte.\n\n' +
        'Javier García Pérez — Administrador — Mat. RPAC N° 83\n\n' + notaRPAC
    }
  }

  // ── Enviar convocatoria por email a todos los copropietarios ───────────────
  async function enviarConvocatoria(asm) {
    const cps = copropietarios.filter(c => c.consorcio_id === consorcioId && c.email)
    if (cps.length === 0) return setMsg({ tipo:'warn', texto:'No hay copropietarios con email registrado en este consorcio' })
    if (!confirm(`¿Enviar convocatoria a ${cps.length} propietarios?`)) return
    setEnviandoConv(true); setMsg(null)
    const { data:{ session:sess } } = await supabase.auth.getSession()
    const texto = generarTextoConvocatoria(asm)
    const tipo = asm.tipo === 'ordinaria' ? 'Ordinaria' : 'Extraordinaria'
    const asunto = `Convocatoria Asamblea General ${tipo} — ${consorcioActivo?.nombre} — ${fmtF(asm.fecha)}`
    let ok = 0, err = 0
    for (const cp of cps) {
      try {
        const r = await fetch(`${SUPA_URL}/functions/v1/enviar-notificacion`, {
          ...
          body: JSON.stringify({ destinatarios:[cp.email], asunto, cuerpo_texto: texto, consorcio_id: consorcioId })
        })
        if (r.ok) ok++; else err++
      } catch { err++ }
    }
    await supabase.from('con_asambleas').update({ convocatoria_enviada:true, fecha_envio_conv: new Date().toISOString(), convocatoria_html: texto }).eq('id', asm.id)
    setMsg({ tipo: err===0?'ok':'warn', texto:'✓ Enviado a ' + ok + ' propietarios' + (err>0?' · ' + err + ' errores':'')})
    setEnviandoConv(false); cargar()
    if (detalle?.id === asm.id) setDetalle({ ...asm, convocatoria_enviada:true })
  }

  // ── Generar acta con IA ────────────────────────────────────────────────────
  async function generarActa(asm) {
    if (!asm.transcripcion?.trim()) return setMsg({ tipo:'warn', texto:'Pegá la transcripción antes de generar el acta' })
    setGenerandoActa(true); setMsg(null)
    const { data:{ session:sess } } = await supabase.auth.getSession()
    const res = await fetch(`${SUPA_URL}/functions/v1/generar-acta-asamblea`, {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'Authorization':`Bearer ${sess?.access_token}`, 'apikey': process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||'' },
      body: JSON.stringify({
        transcripcion:       asm.transcripcion,
        consorcio_nombre:    consorcioActivo?.nombre,
        consorcio_direccion: consorcioActivo?.direccion,
        tipo_asamblea:       asm.tipo,
        modalidad:           asm.modalidad,
        plataforma_virtual:  asm.plataforma_virtual,
        fecha:               fmtF(asm.fecha),
        hora_inicio:         asm.hora,
        hora_fin:            asm.hora_fin,
        orden_del_dia:       asm.orden_del_dia || [],
        unidades_presentes:  asm.unidades_presentes || [],
        total_ufs:           unidades.length,
        pct_presentes:       asm.pct_presentes,
        matricula_rpi:       consorcioActivo?.matricula_rpi,
        escritura_nro:       consorcioActivo?.escritura_nro,
      })
    })
    const data = await res.json()
    if (!res.ok || data.error) {
      setMsg({ tipo:'error', texto:'Error al generar acta: ' + (data.error||'Error desconocido') })
      setGenerandoActa(false); return
    }
    await supabase.from('con_asambleas').update({ acta_borrador: data.acta, estado:'acta_generada', updated_at: new Date().toISOString() }).eq('id', asm.id)
    setDetalle({ ...asm, acta_borrador: data.acta, estado:'acta_generada' })
    setTabDet('acta')
    setMsg({ tipo:'ok', texto:'✓ Acta generada por IA — Revisá y editá antes de aprobar' })
    setGenerandoActa(false); cargar()
  }

  // ── Guardar edición acta ───────────────────────────────────────────────────
  async function guardarActa(asm, texto, aprobar) {
    const updates = aprobar
      ? { acta_final: texto, estado:'acta_aprobada', updated_at: new Date().toISOString() }
      : { acta_borrador: texto, updated_at: new Date().toISOString() }
    await supabase.from('con_asambleas').update(updates).eq('id', asm.id)
    setDetalle({ ...asm, ...updates })
    setMsg({ tipo:'ok', texto: aprobar ? '✓ Acta aprobada y guardada' : '✓ Borrador guardado' })
    cargar()
  }
  ...
  // RENDER FORM
  // ══════════════════════════════════════════════════════════════════════════
  if (tab === 'form' && form) return (
    <div>
      <div style={{ fontWeight:700, fontSize:15, marginBottom:16 }}>
        {form.id ? '✏ Editar asamblea' : `+ Nueva Asamblea ${form.tipo === 'ordinaria' ? 'Ordinaria' : 'Extraordinaria'}`}
      </div>
      <Msg data={msg} />
      {/* Datos generales */}
      <Card style={{ marginBottom:14 }}>
        <div style={{ fontWeight:600, color:AZ, fontSize:13, marginBottom:12 }}>📋 Datos de la asamblea</div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
          <Input label="Fecha" value={form.fecha||''} onChange={v=>setForm(x=>({...x,fecha:v}))} type="date" required />
          ...
          <div>
            ...
            <select value={form.modalidad||'virtual'} onChange={e=>setForm(x=>({...x,modalidad:e.target.value}))}
              style={{ width:'100%', padding:'8px', border:'1px solid #d1d5db', borderRadius:7, fontSize:13 }}>
              {[['presencial','Presencial'],['virtual','Virtual (Zoom/Meet)'],['mixta','Mixta']].map(([v,l])=><option key={v} value={v}>{l}</option>)}
            </select>
          </div>
          {(form.modalidad==='presencial'||form.modalidad==='mixta') && (
            <div style={{ gridColumn: form.modalidad==='mixta' ? '1' : 'span 3' }}>
              <Input label="Lugar" value={form.lugar||''} onChange={v=>setForm(x=>({...x,lugar:v}))} placeholder="SUM del edificio / Salón..." />
            </div>
          )}
          {(form.modalidad==='virtual'||form.modalidad==='mixta') && (<>
            <div>
              <div style={{ fontSize:12, color:GR, marginBottom:4 }}>Plataforma</div>
              <select value={form.plataforma_virtual||'zoom'} onChange={e=>setForm(x=>({...x,plataforma_virtual:e.target.value}))}
                style={{ width:'100%', padding:'8px', border:'1px solid #d1d5db', borderRadius:7, fontSize:13 }}>
                {PLATAFORMAS.map(([v,l])=><option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div style={{ gridColumn:'span 2' }}>
              <Input label="Link de acceso" value={form.link_virtual||''} onChange={v=>setForm(x=>({...x,link_virtual:v}))} placeholder="https://zoom.us/j/..." />
            </div>
            <Input label="Clave de acceso" value={form.clave_virtual||''} onChange={v=>setForm(x=>({...x,clave_virtual:v}))} placeholder="123456" />
          </>)}
        </div>
      </Card>
      {/* Orden del día */}
      <Card style={{ marginBottom:14 }}>
        ...
        {(form.orden_del_dia||[]).map((punto, i) => (
          <div key={i} style={{ display:'flex', gap:8, marginBottom:8, alignItems:'flex-start' }}>
            <div style={{ minWidth:24, paddingTop:9, fontSize:12, color:GR, fontWeight:600 }}>{i+1}.</div>
            <textarea value={punto} onChange={e => {
              const od = [...(form.orden_del_dia||[])]
              od[i] = e.target.value; setForm(x=>({...x,orden_del_dia:od}))
            ...
            <button type="button" onClick={() => {
              const od = (form.orden_del_dia||[]).filter((_,j)=>j!==i)
              setForm(x=>({...x,orden_del_dia:od}))
            }} style={{ padding:'6px 10px', background:'#fee2e2', color:RJ, border:'none', borderRadius:6, cursor:'pointer', fontSize:12, marginTop:4 }}>✕</button>
          </div>
        ))}
        <button type="button" onClick={() => setForm(x=>({...x, orden_del_dia:[...(x.orden_del_dia||[]),'']}))}
          style={{ padding:'7px 16px', background:'#eff6ff', color:AZ, border:`1px solid ${AZ}`, borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer', marginTop:4 }}>
          + Agregar punto
        </button>
      </Card>
      <div style={{ display:'flex', gap:8 }}>
        <Btn onClick={guardar}>💾 Guardar</Btn>
        <BtnSec onClick={()=>{ setTab('lista'); setForm(null) }}>Cancelar</BtnSec>
      </div>
    </div>
  )
  ...
  // ══════════════════════════════════════════════════════════════════════════
  if (detalle) {
    const convTexto = generarTextoConvocatoria(detalle)
    return (
      <DetalleAsamblea
        asm={detalle} consorcioActivo={consorcioActivo}
        unidades={unidades} copropietarios={copropietarios}
        tabDet={tabDet} setTabDet={setTabDet}
        msg={msg} setMsg={setMsg}
        generandoActa={generandoActa} enviandoConv={enviandoConv}
        convTexto={convTexto}
        fmtF={fmtF} ESTADOS_LABEL={ESTADOS_LABEL} ESTADOS_COLOR={ESTADOS_COLOR}
        onVolver={()=>{ setDetalle(null); setTabDet('convocatoria') }}
        onEnviar={()=>enviarConvocatoria(detalle)}
        onGenerarActa={()=>generarActa(detalle)}
        onGuardarActa={(txt,apro)=>guardarActa(detalle,txt,apro)}
        onEditar={()=>{ setForm({...detalle, orden_del_dia: detalle.orden_del_dia||[]}); setTab('form') }}
        onActualizar={(updates)=>setDetalle(d=>({...d,...updates}))}
  ...
  return (
    <div>
      ...
      <div style={{ fontSize:12, color:GR, marginBottom:16 }}>
        Convocatorias, actas y seguimiento de asambleas del consorcio
      </div>
      <Msg data={msg} />
      {asambleas.length === 0 ? (
        <Card>
          <div style={{ textAlign:'center', padding:'32px 0', color:GR }}>
            <div style={{ fontSize:40, marginBottom:10 }}>🏛</div>
            <div style={{ fontWeight:600, marginBottom:6 }}>Sin asambleas registradas</div>
            <div style={{ fontSize:12 }}>Creá una nueva convocatoria para comenzar</div>
          </div>
        </Card>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {asambleas.map(a => {
            ...
            const esProxima = a.estado === 'convocatoria' && dias !== null && dias >= 0
            return (
              <Card key={a.id} onClick={()=>{ setDetalle(a); setTabDet('convocatoria') }}
                style={{ cursor:'pointer', border: esProxima ? `1.5px solid ${AZ}` : '1px solid #e5e7eb',
                  background: esProxima ? '#f8fbff' : '#fff' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                      <span style={{ fontWeight:700, fontSize:13 }}>
                        Asamblea {a.tipo === 'ordinaria' ? 'General Ordinaria' : 'General Extraordinaria'}
                      </span>
                      <span style={{ fontSize:11, padding:'2px 8px', borderRadius:6, fontWeight:600,
                        background: a.estado==='acta_aprobada'?'#dcfce7':a.estado==='acta_generada'?'#ede9fe':a.estado==='realizada'?'#fff8e1':'#eff6ff',
                        color: ESTADOS_COLOR[a.estado] }}>
                        {ESTADOS_LABEL[a.estado]}
                      </span>
                    </div>
                    <div style={{ fontSize:12, color:GR }}>
                      {fmtF(a.fecha)} — {a.hora} hs ·
                      {a.modalidad === 'virtual' ? ` Virtual (${a.plataforma_virtual||'Zoom'})` :
                       a.modalidad === 'mixta'   ? ` Presencial + Virtual` : ` Presencial`}
                      {a.lugar ? ` · ${a.lugar}` : ''}
                    </div>
                    {a.orden_del_dia?.length > 0 && (
                      <div style={{ fontSize:11, color:GR, marginTop:4 }}>
                        {a.orden_del_dia.length} puntos en Orden del Día
                        {a.convocatoria_enviada ? ' · ✉ Convocatoria enviada' : ''}
                        {a.acta_final ? ' · ✔ Acta aprobada' : a.acta_borrador ? ' · 📄 Borrador de acta' : ''}
                      </div>
                    )}
                  </div>
                  <div style={{ textAlign:'right', marginLeft:12 }}>
                    {esProxima && (
                      <div style={{ fontSize:16, fontWeight:800, color: dias<=7?RJ:dias<=15?AM:AZ }}>
                        {dias === 0 ? '¡Hoy!' : `${dias}d`}
                      </div>
                    )}
...
// Sub-componente Detalle para mantener el render principal limpio
function DetalleAsamblea({ asm, consorcioActivo, unidades, copropietarios, tabDet, setTabDet,
  msg, setMsg, generandoActa, enviandoConv, convTexto, fmtF,
  ESTADOS_LABEL, ESTADOS_COLOR, onVolver, onEnviar, onGenerarActa,
  onGuardarActa, onEditar, onActualizar, supabase, session }) {

  const [actaEdit, setActaEdit]       = useState(asm.acta_borrador || asm.acta_final || '')
  const [transcEdit, setTranscEdit]   = useState(asm.transcripcion || '')
  const [presentesText, setPresentesText] = useState((asm.unidades_presentes||[]).join(', '))
  const [horaFin, setHoraFin]         = useState(asm.hora_fin || '')

  useEffect(() => {
    setActaEdit(asm.acta_borrador || asm.acta_final || '')
    setTranscEdit(asm.transcripcion || '')
  }, [asm.id])

  async function guardarTranscripcion() {
    const presentes = presentesText.split(',').map(p=>p.trim()).filter(Boolean)
    await supabase.from('con_asambleas').update({
      transcripcion: transcEdit,
      unidades_presentes: presentes,
      total_presentes: presentes.length,
      hora_fin: horaFin,
      estado: 'realizada',
      updated_at: new Date().toISOString(),
    }).eq('id', asm.id)
    onActualizar({ transcripcion: transcEdit, unidades_presentes: presentes, hora_fin: horaFin, estado:'realizada' })
    setMsg({ tipo:'ok', texto:'✓ Transcripción guardada' })
  }

  const tabs = [
    { id:'convocatoria', l:'📋 Convocatoria' },
    { id:'asistencia',   l:'👥 Asistencia' },
    { id:'transcripcion',l:'🎙 Transcripción' },
    { id:'acta',         l:'📄 Acta' },
  ]
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
        <BtnSec onClick={onVolver}>← Volver</BtnSec>
        <div style={{ flex:1 }}>
          <div style={{ fontWeight:700, fontSize:15 }}>
            Asamblea {asm.tipo==='ordinaria'?'General Ordinaria':'General Extraordinaria'}
          </div>
          <div style={{ fontSize:12, color:GR }}>
            {fmtF(asm.fecha)} — {asm.hora} hs ·
            <span style={{ marginLeft:6, fontWeight:600, color:ESTADOS_COLOR[asm.estado] }}>
              {ESTADOS_LABEL[asm.estado]}
            </span>
          </div>
        </div>
        <BtnSec onClick={onEditar} style={{ fontSize:12 }}>✏ Editar</BtnSec>
      </div>
      <Msg data={msg} />
      {/* Tabs */}
      <div style={{ display:'flex', gap:0, marginBottom:16, borderBottom:'2px solid #e5e7eb', overflowX:'auto' }}>
        {tabs.map(t=>(
          <button key={t.id} type="button" onClick={()=>setTabDet(t.id)}
            style={{ padding:'8px 14px', border:'none', borderBottom:tabDet===t.id?`2px solid ${AZ}`:'2px solid transparent',
              background:'transparent', color:tabDet===t.id?AZ:GR, fontWeight:tabDet===t.id?700:400,
              fontSize:12.5, cursor:'pointer', marginBottom:-2, whiteSpace:'nowrap' }}>{t.l}
          </button>
        ))}
      </div>
      ...
      {tabDet === 'convocatoria' && (
        <div>
          <Card style={{ marginBottom:14 }}>
            ...
            {(asm.orden_del_dia||[]).map((p,i)=>(
              <div key={i} style={{ display:'flex', gap:8, marginBottom:6, fontSize:13 }}>
                <span style={{ color:GR, fontWeight:600, minWidth:20 }}>{i+1}.</span>
                <span>{p}</span>
              </div>
            ))}
          </Card>
          {(asm.modalidad==='virtual'||asm.modalidad==='mixta') && asm.link_virtual && (
            <Card style={{ marginBottom:14, background:'#eff6ff', border:`1px solid #bfdbfe` }}>
              ...
              <div style={{ fontSize:13 }}>
                <a href={asm.link_virtual} target="_blank" rel="noreferrer" style={{ color:AZ, fontWeight:600 }}>
                  {asm.link_virtual}
                </a>
              </div>
              {asm.clave_virtual && <div style={{ fontSize:12, color:GR, marginTop:4 }}>Clave: {asm.clave_virtual}</div>}
            </Card>
          )}
          <Card style={{ marginBottom:14 }}>
            <div style={{ fontWeight:600, color:AZ, fontSize:13, marginBottom:8 }}>Texto de la convocatoria</div>
            <textarea readOnly value={convTexto} rows={20}
              style={{ width:'100%', padding:'12px', border:'1px solid #e5e7eb', borderRadius:8, fontSize:12,
                fontFamily:'Georgia, serif', lineHeight:1.8, background:'#fafafa', resize:'vertical', boxSizing:'border-box' }} />
          </Card>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            <Btn onClick={() => { navigator.clipboard.writeText(convTexto); setMsg({ tipo:'ok', texto:'✓ Texto copiado' }) }}>
              📋 Copiar texto
            </Btn>
            <Btn onClick={onEnviar} disabled={enviandoConv} style={{ opacity:enviandoConv?0.5:1, background:'#16a34a' }}>
              {enviandoConv ? '⏳ Enviando...' : `✉ Enviar a propietarios${asm.convocatoria_enviada?' (re-enviar)':''}`}
            </Btn>
          </div>
          {asm.convocatoria_enviada && (
            <div style={{ marginTop:10, fontSize:12, color:VD }}>
              ✓ Convocatoria enviada el {fmtF(asm.fecha_envio_conv?.split('T')[0])}
            </div>
          )}
        </div>
      )}
      ...
      {tabDet === 'asistencia' && (
        <div>
          <Card style={{ marginBottom:14 }}>
            <div style={{ fontWeight:600, color:AZ, fontSize:13, marginBottom:12 }}>Registro de asistencia</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
              <div>
                <div style={{ fontSize:12, color:GR, marginBottom:4 }}>Unidades/depts. presentes</div>
                <textarea value={presentesText} onChange={e=>setPresentesText(e.target.value)} rows={6}
                  placeholder="Ej: 1A, 2B, 3C, PB-E, 5-F TII..."
                  style={{ width:'100%', padding:'10px', border:'1px solid #d1d5db', borderRadius:7, fontSize:12.5, fontFamily:'inherit', resize:'vertical', boxSizing:'border-box' }} />
                <div style={{ fontSize:11, color:GR, marginTop:4 }}>{presentesText.split(',').filter(p=>p.trim()).length} unidades ingresadas</div>
              </div>
              <div>
                <div style={{ fontSize:12, color:GR, marginBottom:8, fontWeight:500 }}>Resumen</div>
                <div style={{ background:'#f8fafc', borderRadius:8, padding:'12px', fontSize:13 }}>
                  <div style={{ marginBottom:6 }}>Total UFs consorcio: <strong>{unidades.length}</strong></div>
                  <div style={{ marginBottom:6 }}>Presentes: <strong>{presentesText.split(',').filter(p=>p.trim()).length}</strong></div>
                  <div style={{ marginBottom:6 }}>Porcentaje: <strong style={{ color: presentesText.split(',').filter(p=>p.trim()).length/unidades.length > 0.5 ? VD : RJ }}>
                    {unidades.length > 0 ? ((presentesText.split(',').filter(p=>p.trim()).length/unidades.length)*100).toFixed(1) : 0}%
                  </strong></div>
                  <div style={{ marginBottom:6 }}>Hora inicio: <strong>{asm.hora}</strong></div>
                  <input type="time" value={horaFin} onChange={e=>setHoraFin(e.target.value)}
                    placeholder="Hora fin" style={{ padding:'6px', border:'1px solid #d1d5db', borderRadius:6, fontSize:13, marginTop:4, width:'100%' }} />
                  <div style={{ fontSize:11, color:GR, marginTop:4 }}>Hora de finalización</div>
                </div>
              </div>
            </div>
      ...
      {/* ── TAB TRANSCRIPCIÓN ── */}
      {tabDet === 'transcripcion' && (
        <div>
          <div style={{ fontSize:12, color:GR, marginBottom:10 }}>
            Pegá la transcripción de la asamblea (texto de Zoom, Meet, o transcripción manual). La IA la analizará para generar el acta formal.
          </div>
          <Card style={{ marginBottom:14 }}>
            <textarea value={transcEdit} onChange={e=>setTranscEdit(e.target.value)} rows={22}
              placeholder="Pegar aquí la transcripción completa de la asamblea..."
              style={{ width:'100%', padding:'12px', border:'1px solid #d1d5db', borderRadius:8, fontSize:12.5,
                fontFamily:'inherit', lineHeight:1.7, resize:'vertical', boxSizing:'border-box' }} />
            <div style={{ display:'flex', gap:8, marginTop:10 }}>
              <Btn onClick={guardarTranscripcion}>💾 Guardar transcripción</Btn>
              <Btn onClick={onGenerarActa} disabled={generandoActa||!transcEdit.trim()}
                style={{ background:'#7c3aed', opacity:generandoActa||!transcEdit.trim()?0.5:1 }}>
                {generandoActa ? '⏳ Generando acta...' : '🤖 Generar acta con IA'}
              </Btn>
            </div>
          </Card>
        </div>
      ...
      {tabDet === 'acta' && (
        <div>
          {!actaEdit && !asm.acta_borrador ? (
            <Card>
              <div style={{ textAlign:'center', padding:'24px 0', color:GR }}>
                <div style={{ fontSize:32, marginBottom:8 }}>📄</div>
                <div style={{ fontWeight:600, marginBottom:6 }}>Sin acta generada</div>
                <div style={{ fontSize:12, marginBottom:16 }}>Primero cargá la transcripción en la pestaña anterior y generá el acta con IA</div>
            ...
            <div>
              <div style={{ fontSize:12, color:GR, marginBottom:10 }}>
                {asm.acta_final ? '✔ Acta aprobada — editá si necesitás corregir algo' : '📄 Borrador generado por IA — revisá y editá antes de aprobar'}
              </div>
              <textarea value={actaEdit} onChange={e=>setActaEdit(e.target.value)} rows={30}
                style={{ width:'100%', padding:'14px', border:`1px solid ${asm.acta_final?VD:'#d1d5db'}`, borderRadius:8,
                  fontSize:13, fontFamily:'Georgia, serif', lineHeight:1.9, resize:'vertical', boxSizing:'border-box',
                  background: asm.acta_final?'#f0fdf4':'#fff' }} />
              <div style={{ display:'flex', gap:8, marginTop:10, flexWrap:'wrap' }}>
                <Btn onClick={()=>onGuardarActa(actaEdit,false)}>💾 Guardar borrador</Btn>
                {!asm.acta_final && (
                  <Btn onClick={()=>{ if(confirm('¿Aprobar el acta definitivamente?')) onGuardarActa(actaEdit,true) }}
                    style={{ background:VD }}>
                    ✔ Aprobar acta
                  </Btn>
                )}
                <Btn onClick={()=>{ navigator.clipboard.writeText(actaEdit); setMsg({ tipo:'ok', texto:'✓ Acta copiada al portapapeles' }) }}
                  style={{ background:'#374151' }}>
                  📋 Copiar acta
                </Btn>
                <Btn onClick={onGenerarActa} disabled={generandoActa} style={{ background:'#7c3aed', opacity:generandoActa?0.5:1 }}>
                  {generandoActa ? '⏳ Regenerando...' : '🤖 Regenerar con IA'}
                </Btn>
              </div>
            </div>
          )}
...
function EmailTracking({ session, consorcioId }) {
  ...
  async function verificarTodos() {
    ...
    setMsg({ tipo:'ok', texto:`✓ ${actualizados} emails verificados — recargando...` })
    await cargar()
    setMsg({ tipo:'ok', texto:`✓ Verificación completa — ${actualizados} emails actualizados` })
  }

  useEffect(() => {
    if (consorcioId) {
      cargar()
      // Auto-verificar emails recientes sin apertura (en background, silencioso)
      setTimeout(() => verificarTodos(), 1500)
    }
  }, [consorcioId])
                        ...
                        {log.abierto
                          ...
                          : log.entregado
                            ...
                            : log.rebotado
                              ...
                              : <span style={{ color:GR, fontSize:11 }}>⏳ Sin verificar</span>
                        }
                      </td>
...
function CuentasBancarias({ session, consorcioId, consorcioActivo }) {
  ...
  function handlePDFCuentas(){
    exportarPDF({titulo:'Cuentas Bancarias',logoB64:LOGO_ADM_B64,
...
function MovEntrecuentas({ session, consorcioId }) {
  ...
  function handlePDF(){
    exportarPDF({titulo:'Movimientos entre Cuentas',logoB64:LOGO_ADM_B64,
...
function MovimientosVarios({ session, consorcioId, expensas }) {
  ...
  function handlePDF() {
    exportarPDF({titulo:'Movimientos Varios',logoB64:LOGO_ADM_B64,
...
// ══════════════════════════════════════════════════════════════════════════════
// SUELDOS — Gestión de empleados y liquidación de sueldos
// Carga manual + lectura IA de recibo PDF
// Imputación automática a con_gastos (Rubro 2: SUELDOS Y CARGAS SOCIALES)
// Referencia: CCT 589/10 FATERYH · Punta Médanos como ejemplo
// ══════════════════════════════════════════════════════════════════════════════
function Sueldos({ session, consorcioId, consorcioActivo, expensas }) {
...
// ── FormComp fuera del componente: evita re-mount en cada keystroke ─────────
...
// ── FormPago fuera del componente: evita re-mount en cada keystroke ─────────
...
function CtaProveedor({ session, consorcioId, proveedores }) {
  ...
  function handlePDFCta(){
    exportarPDF({titulo:`Cuenta Corriente — ${prov?.razon_social||'Proveedor'}`,logoB64:LOGO_ADM_B64,
...
// ══════════════════════════════════════════════════════════════════════════════
// LISTADO DE CONSORCIOS — búsqueda, PDF y Excel
// ══════════════════════════════════════════════════════════════════════════════
function ListadoConsorcios({ session, consorcios }) {
  const [busqueda, setBusqueda] = useState('')
  ...
  function handlePDF() {
    ...
    const fmtN = n => (Number(n)||0).toLocaleString('es-AR', { minimumFractionDigits:2 })
    const logo = LOGO_ADM_B64 ? `<img src="${LOGO_ADM_B64}" style="height:44px;width:auto;object-fit:contain"/>` : ''
      ...
      <div>
        ...
        {formCon && (
          <Card style={{ marginBottom:20, border:`1px solid ${AZ}` }}>
            ...
            <div style={{ fontWeight:600, color:GR, fontSize:12, marginBottom:10, textTransform:'uppercase', letterSpacing:'0.5px' }}>📖 Reglamento de propiedad</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
              <Input label="Matrícula RPI" value={formCon.matricula_rpi||''} onChange={v=>setFormCon(x=>({...x,matricula_rpi:v}))} placeholder="21.525" />
              <Input label="Escritura N°" value={formCon.escritura_nro||''} onChange={v=>setFormCon(x=>({...x,escritura_nro:v}))} placeholder="395" />
              <div style={{ gridColumn:'span 2' }}>
                <Input label="Fecha escritura" value={formCon.escritura_fecha||''} onChange={v=>setFormCon(x=>({...x,escritura_fecha:v}))} placeholder="19 de mayo de 1994" />
              </div>
              <div style={{ gridColumn:'span 2' }}>
                <Input label="Escribano/a autorizante" value={formCon.escritura_escribano||''} onChange={v=>setFormCon(x=>({...x,escritura_escribano:v}))} placeholder="Horacio F. Ballestrin" />
              </div>
            </div>
            <div style={{ fontWeight:600, color:GR, fontSize:12, marginBottom:10, textTransform:'uppercase', letterSpacing:'0.5px' }}>🛡️ Póliza de seguro</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
              <Input label="N° de póliza" value={formCon.poliza_nro||''} onChange={v=>setFormCon(x=>({...x,poliza_nro:v}))} placeholder="250230763671" />
              <div style={{ gridColumn:'span 1' }}>
                <Input label="Compañía aseguradora" value={formCon.poliza_compania||''} onChange={v=>setFormCon(x=>({...x,poliza_compania:v}))} placeholder="Allianz Argentina" />
              </div>
              <Input label="Vigencia desde" value={formCon.poliza_vto_desde||''} onChange={v=>setFormCon(x=>({...x,poliza_vto_desde:v}))} type="date" />
              <Input label="Vigencia hasta" value={formCon.poliza_vto_hasta||''} onChange={v=>setFormCon(x=>({...x,poliza_vto_hasta:v}))} type="date" />
              <div style={{ gridColumn:'span 2' }}>
                <Input label="Suma asegurada incendio edificio ($)" value={formCon.poliza_suma||''} onChange={v=>setFormCon(x=>({...x,poliza_suma:v}))} type="number" placeholder="720000000" />
              </div>
            </div>
            <div style={{ fontWeight:600, color:GR, fontSize:12, marginBottom:10, textTransform:'uppercase', letterSpacing:'0.5px' }}>📁 Google Drive</div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:16 }}>
              <div style={{ gridColumn:'span 2' }}>
                <Input label="URL carpeta Drive del consorcio" value={formCon.drive_folder_url||''} onChange={v=>setFormCon(x=>({...x,drive_folder_url:v}))} placeholder="https://drive.google.com/drive/folders/..." />
              </div>
              <div style={{ gridColumn:'span 2' }}>
                <Input label="ID carpeta Drive (último segmento de la URL)" value={formCon.drive_folder_id||''} onChange={v=>setFormCon(x=>({...x,drive_folder_id:v}))} placeholder="1EXlLiRq6G43qzwLvLGR_UkM-TB9BeH58" />
              </div>
            </div>
            <div style={{ display:'flex', gap:8 }}>
...
export default function App() {
 ...
 const NAV=[
    ...
    { id:'sueldos',             label:'Sueldos',                 icon:'💼', sec:'Consorcio' },
    ...
    { id:'asambleas',           label:'Asambleas',                icon:'🏛', sec:'Consorcio' },
    { id:'reclamos',           label:'Reclamos / Tickets',       icon:'🎫', sec:'Consorcio' },
    { id:'agenda_venc',         label:'Agenda vencimientos',      icon:'📅', sec:'Consorcio' },

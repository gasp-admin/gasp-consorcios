...
const BUILD_VERSION = '20260520-sueldos-v22'
...
function CobranzasAutomaticas({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
  ...
  function parseMtoLocal(s) {
    if (!s) return 0
    return parseFloat(s.toString().replace(/\$/g,'').replace(/\s/g,'').replace(/\./g,'').replace(',','.')) || 0
  }
...
function Sueldos({ session, consorcioId, consorcioActivo, expensas }) {
  ...
  const hoy = new Date().toISOString().split('T')[0]
  const fmt = n => '$' + (Number(n)||0).toLocaleString('es-AR', { minimumFractionDigits:2 })
  const parseMto = s => parseFloat(String(s||'0').replace(/\./g,'').replace(',','.')) || 0
  ...
  function actualizarFila(idx, campo, valor) {
    setFilas(prev => prev.map((f, i) => {
      ...
      if (['sueldo_basico','adicionales','aportes_empleado'].includes(campo)) {
        ...
        upd.sueldo_neto = (bas + adi - apo || 0).toFixed(2)
      }
      return upd
    }))
  }

  // ── Lectura IA de recibo PDF ─────────────────────────────────────────
  async function leerReciboConIA(file) {
    if (!file) return
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              {
                type: 'text',
                text: `Analiza este recibo de sueldo de empleado de consorcio de propiedad horizontal argentino (CCT 589/10 FATERYH) y extrae los datos en JSON puro sin markdown.
Devuelve SOLO este JSON (sin explicaciones):
{
  "apellido_nombre": "APELLIDO Nombre completo",
  "cuil": "XX-XXXXXXXX-X o vacío",
  "periodo": "YYYY-MM (mes al que corresponde el sueldo)",
  "tipo_liquidacion": "mensual | final | reemplazo | adicional",
  "es_liquidacion_final": false,
  "sueldo_basico": 0.00,
  "adicionales": 0.00,
  "aportes_empleado": 0.00,
  "sueldo_neto": 0.00,
  "columna": "GENERAL",
  "notas": "descripción breve si hay algo especial (reemplazo, guardia, etc.)"
}
Para columna: usar GENERAL para portería/limpieza general del edificio, DEPTOS para personal asignado a departamentos, LOCALES para personal asignado a locales, COCHERAS para cocheras.
Si es un recibo de reemplazo/guardia, tipo_liquidacion='reemplazo'.
Si dice 'Liquidación final' o 'baja', es_liquidacion_final=true y tipo_liquidacion='final'.`
              }
            ]
          }]
        })
      ...
      // Crear fila con los datos extraídos
      const nuevaFila = {
        empleado_id:         null,
      ...
      // Si el período del recibo difiere del expSel, avisar
      if (parsed.periodo && expSel) {
        const expPer = expensas.find(e=>e.id===expSel)?.periodo
        if (expPer && parsed.periodo !== expPer) {
          nuevaFila.notas = (nuevaFila.notas ? nuevaFila.notas + ' | ' : '') + `⚠ Período recibo: ${parsed.periodo}`
        }
      }
      // Buscar empleado coincidente
      const match = empleados.find(e =>
        e.apellido_nombre.toUpperCase().includes(parsed.apellido_nombre?.split(' ')[0]?.toUpperCase() || 'XXXXXX')
      )
      if (match) nuevaFila.empleado_id = match.id

      setFilas(prev => [...prev, nuevaFila])
      setMsg({ tipo:'ok', texto:`✓ IA extrajo datos de "${file.name}" — Verificar antes de imputar` })
    } catch(e) {
      setMsg({ tipo:'error', texto:'Error al leer PDF: ' + e.message })
    }
    setLeyendoIA(false)
  }
  ...
  async function imputarSueldos() {
    ...
    for (const fila of filasActivas) {
      ...
      // Mapear columna → campo de con_gastos
      const colMap = {
        'GENERAL':   { pct_grupo_a:100 },
        'LOCALES':   { pct_locales:100 },
        'DEPTOS':    { pct_dptos:100 },
        'COCHERAS':  { pct_cocheras:100 },
        'FDO_OBRAS': { pct_fdo_obras:100 },
      }
      const colGasto = colMap[fila.columna] || { pct_grupo_a:100 }
    ...
    if (montoFateryh > 0) {
      ...
      const { error: ef } = await supabase.from('con_gastos').insert([{
        ...
        monto: montoFateryh, fecha: hoy, pct_grupo_a:100,
      }])
    ...
    if (montoVep > 0) {
      ...
      const { error: ev } = await supabase.from('con_gastos').insert([{
        ...
        monto: montoVep, fecha: hoy, pct_grupo_a:100,
      ...
      {tab === 'liquidar' && (
        <div>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap' }}>
            <Btn small onClick={agregarFilaManual}>+ Agregar sueldo manual</Btn>
            <div>
              <input
                type="file" accept=".pdf,application/pdf"
                onChange={e => { if(e.target.files[0]) leerReciboConIA(e.target.files[0]) }}
                style={{ display:'none' }} id="ia-recibo-upload" />
              <label htmlFor="ia-recibo-upload">
                <button
                  disabled={leyendoIA}
                  onClick={() => document.getElementById('ia-recibo-upload').click()}
                  style={{ padding:'6px 14px', background:'#7c3aed', color:'#fff', border:'none',
                    borderRadius:7, fontSize:12.5, fontWeight:600, cursor:'pointer',
                    opacity:leyendoIA?0.6:1 }}>
                  {leyendoIA ? '⏳ Leyendo PDF...' : '🤖 Leer recibo con IA (PDF)'}
                </button>
              </label>
            </div>
          </div>

...
const BUILD_VERSION = '20260524-FINAL-v46'
...
// ══════════════════════════════════════════════════════════════════════════════
// ASAMBLEAS — Convocatorias, Actas IA, Registro (v4 - const arrows, TDZ safe)
// ══════════════════════════════════════════════════════════════════════════════
function Asambleas({ session, consorcioId, consorcioActivo, unidades, copropietarios, expensas }) {
  ...
  const NOTA_RP = 'Nota RPAC (art.2060 CCCN): para ser decisión: más del 50% consorcistas + 50% propiedad (doble mayoría). Si no, es "propuesta" notificable a ausentes en 15 días. No hay primer ni segundo llamado (Ley 14.701). Documentar resultado de cada punto (art.2062 CCCN).'
  ...
  // abrirDetalle DESPUÉS de generarTexto (evita TDZ)
  ...
  const guardarForm = async () => {
    ...
    const { error } = await supabase.from('con_asambleas').upsert([{
      ...form, id: form.id||('ASM-'+consorcioId+'-'+Date.now()),
      admin_id: session.user.id, consorcio_id: consorcioId,
    ...
    <div>
      ...
      <Card style={{marginBottom:14}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
          ...
          {(form.modalidad==='presencial'||form.modalidad==='mixta')&&(<div style={{gridColumn:'span 3'}}><Input label="Lugar presencial" value={form.lugar||''} onChange={v=>setForm(x=>({...x,lugar:v}))} placeholder="SUM del edificio..."/></div>)}
  ...
  if (vista === 'detalle' && detalle) return (
    <div>
      ...
      <div style={{display:'flex',gap:0,marginBottom:16,borderBottom:'2px solid #e5e7eb',overflowX:'auto'}}>
        {[['convocatoria','📋 Convocatoria'],['asistencia','👥 Asistencia'],['transcripcion','🎙 Transcripción'],['acta','📄 Acta']].map(([id,l])=>(
          <button key={id} type="button" onClick={()=>setTabDet(id)}
            style={{padding:'8px 14px',border:'none',borderBottom:tabDet===id?'2px solid '+AZ:'2px solid transparent',background:'transparent',color:tabDet===id?AZ:GR,fontWeight:tabDet===id?700:400,fontSize:12.5,cursor:'pointer',marginBottom:-2,whiteSpace:'nowrap'}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tabDet==='convocatoria'&&(<div>
        {(detalle.orden_del_dia||[]).length>0&&(
          <Card style={{marginBottom:14}}>
            ...
            {(detalle.orden_del_dia||[]).map((p,i)=>(
              <div key={i} style={{display:'flex',gap:8,marginBottom:5,fontSize:13}}><span style={{color:GR,fontWeight:600,minWidth:20}}>{i+1}.</span><span>{p}</span></div>
            ))}
          </Card>
        )}
        ...
        <Card style={{marginBottom:14}}>
          <div style={{fontWeight:600,color:AZ,fontSize:13,marginBottom:8}}>Texto de la convocatoria <span style={{fontWeight:400,color:GR,fontSize:11}}>(editable antes de enviar)</span></div>
          <textarea value={convTxt} onChange={e=>setConvTxt(e.target.value)} rows={22}
            style={{width:'100%',padding:'12px',border:'1px solid #e5e7eb',borderRadius:8,fontSize:12,fontFamily:'Georgia, serif',lineHeight:1.8,background:'#fafafa',resize:'vertical',boxSizing:'border-box'}}/>
        </Card>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          <Btn onClick={()=>{setConvTxt(generarTexto(detalle));setMsg({tipo:'ok',texto:'✓ Texto regenerado'})}}>🔄 Regenerar</Btn>
          <Btn onClick={()=>{navigator.clipboard.writeText(convTxt);setMsg({tipo:'ok',texto:'✓ Copiado'})}}>📋 Copiar texto</Btn>
        ...
        <Card style={{marginBottom:14}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
            ...
            <div style={{background:'#f8fafc',borderRadius:8,padding:'14px',fontSize:13}}>
              ...
              <div style={{marginBottom:12}}>Porcentaje: <strong style={{color:presentes.split(',').filter(p=>p.trim()).length/Math.max(unidades.length,1)>0.5?VD:RJ}}>{unidades.length>0?((presentes.split(',').filter(p=>p.trim()).length/unidades.length)*100).toFixed(1):0}%</strong></div>
      ...
      {tabDet==='transcripcion'&&(<div>
        <div style={{fontSize:12,color:GR,marginBottom:10}}>Pegá la transcripción completa (de Zoom, Meet, o manual). La IA generará el acta formal.</div>
        <Card style={{marginBottom:14}}>
          <textarea value={transcEdit} onChange={e=>setTranscEdit(e.target.value)} rows={22} placeholder="Transcripción completa de la asamblea..."
            style={{width:'100%',padding:'12px',border:'1px solid #d1d5db',borderRadius:8,fontSize:12.5,fontFamily:'inherit',lineHeight:1.7,resize:'vertical',boxSizing:'border-box'}}/>
        ...
        {!actaEdit&&!detalle.acta_borrador?(
          ...
          <div>
            ...
            <textarea value={actaEdit} onChange={e=>setActaEdit(e.target.value)} rows={32}
              style={{width:'100%',padding:'14px',border:'1px solid '+(detalle.acta_final?VD:'#d1d5db'),borderRadius:8,fontSize:13,fontFamily:'Georgia, serif',lineHeight:1.9,resize:'vertical',boxSizing:'border-box',background:detalle.acta_final?'#f0fdf4':'#fff'}}/>
            <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
              ...
              <Btn onClick={()=>{navigator.clipboard.writeText(actaEdit);setMsg({tipo:'ok',texto:'✓ Copiado'})}} style={{background:'#374151'}}>📋 Copiar</Btn>
          ...
          {asambleas.map(a=>{
            ...
            return (
              <Card key={a.id} onClick={()=>abrirDetalle(a)}
                style={{cursor:'pointer',border:esP?'1.5px solid '+AZ:'1px solid #e5e7eb',background:esP?'#f8fbff':'#fff'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
                    ...
                    <div style={{fontSize:12,color:GR}}>
                      ...
                      {a.modalidad==='virtual'?' Virtual ('+(a.plataforma_virtual||'Zoom')+')':a.modalidad==='mixta'?' Presencial + Virtual':' Presencial'}
                      {a.lugar?' · '+a.lugar:''}
                    </div>
                    <div style={{fontSize:11,color:GR,marginTop:4}}>

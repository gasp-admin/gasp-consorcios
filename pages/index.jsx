...
const BUILD_VERSION = '20260524-FINAL-v50'
...
function HistorialLiquidaciones({ session, consorcioId, consorcioActivo, consorcios }) {
  ...
  const sbGet = async (tabla, query='') => {
    const r = await fetch(`${SB}/rest/v1/${tabla}?${query}`, { headers: { apikey: AK, Authorization: `Bearer ${tok}`, 'Content-Type':'application/json' }});
    return r.json();
  };
  ...
  const cargar = async () => {
    ...
    try {
      const [h, c] = await Promise.all([
        sbGet('con_expensas', `fuente=neq.gasp&estado=eq.cerrado&pdf_procesado=eq.true&select=id,consorcio_id,periodo,total_gastos,saldo_caja_final,fecha_vencimiento,drive_pdf_url&order=periodo.desc&limit=200`).catch(() => []),
        fetch(`${EF_URL}`, { method:'POST', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${tok}` }, body: JSON.stringify({ accion:'estado_cola' }) }).then(r=>r.json()).catch(() => ({ cola:[] }))
      ]);
      setHistorial(Array.isArray(h) ? h : []);
      setCola(Array.isArray(c.cola) ? c.cola : []);
    } finally { setLoading(false); }
  };

  // Cargar también las propias (fuente gasp con pdf)
  const cargarTodo = async () => {
    setLoading(true);
    try {
      const [hist1, hist2, c] = await Promise.all([
        sbGet('con_expensas', `pdf_procesado=eq.true&select=id,consorcio_id,periodo,total_gastos,saldo_caja_final,fecha_vencimiento,drive_pdf_url,fuente&order=periodo.desc&limit=300`).catch(()=>[]),
        sbGet('con_liquidacion_items', `select=expensa_id,consorcio_id,periodo&order=periodo.desc&limit=10`).catch(()=>[]),
        fetch(`${EF_URL}`, { method:'POST', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${tok}` }, body: JSON.stringify({ accion:'estado_cola' }) }).then(r=>r.json()).catch(()=>({ cola:[] }))
      ]);
      setHistorial(Array.isArray(hist1) ? hist1 : []);
      setCola(Array.isArray(c.cola) ? c.cola : []);
    } finally { setLoading(false); }
  };
  ...
  // Extraer ID de carpeta Drive desde URL
  const extraerFolderId = (url) => {
    const m = url.match(/\/folders\/([a-zA-Z0-9_-]+)/);
    return m ? m[1] : null;
  };

  // Buscar PDFs de liquidaciones en carpeta Drive
  const buscarEnDrive = async () => {
    const folderId = extraerFolderId(driveUrl);
    if (!folderId) { setMsg('❌ URL de carpeta no válida. Debe ser del tipo drive.google.com/drive/folders/...'); return; }
    setLoading(true); setMsg('🔍 Buscando liquidaciones en Drive...');
    try {
      // Usar la API de Drive via fetch (requiere token de usuario)
      // Alternativamente usar URL pública si la carpeta es compartida
      const query = `parentId='${folderId}' AND mimeType='application/pdf'`;
      ...
      if (d.archivos?.length) {
        // Filtrar solo los que parecen liquidaciones
        const liqKeywords = ['liq', 'expensa', 'liquidac', 'periodo', 'LIQ', 'EXPENSA'];
        const filtrados = d.archivos.filter((a) =>
          liqKeywords.some(k => a.nombre.toLowerCase().includes(k.toLowerCase()))
        );
        setArchivosEncontrados(filtrados.length ? filtrados : d.archivos);
        setSeleccionados(filtrados.map(a => a.id));
        setMsg(`✅ ${filtrados.length} archivo(s) de liquidación encontrados`);
      } else {
        setMsg('⚠️ No se encontraron PDFs en esa carpeta. Verificar que sea la carpeta correcta y que esté compartida.');
      }
    } catch(e) {
  ...
  // Encolar archivos seleccionados y procesar uno a uno
  const procesarSeleccionados = async () => {
    if (!seleccionados.length) { setMsg('Seleccionar al menos un archivo'); return; }
    if (!consorcioActivo?.id) { setMsg('Seleccionar un consorcio activo'); return; }
    setProcesando(true);
    setProgreso({ actual: 0, total: seleccionados.length });
    setMsg(`⚙️ Procesando ${seleccionados.length} liquidaciones...`);

    let ok = 0, err = 0;
    for (let i = 0; i < seleccionados.length; i++) {
      const fileId = seleccionados[i];
      const archivo = archivosEncontrados.find(a => a.id === fileId);
      setProgreso({ actual: i + 1, total: seleccionados.length });
      setMsg(`⚙️ Procesando ${i+1}/${seleccionados.length}: ${archivo?.nombre ?? fileId}`);
      ...
      await fetch(`${EF_URL}`, {
        ...
        body: JSON.stringify({
          accion: 'encolar_lote',
          archivos: [{ drive_file_id: fileId, drive_file_nombre: archivo?.nombre, consorcio_id: consorcioActivo.id, consorcio_nombre: consorcioActivo.nombre }]
        })
      });
      ...
      if (res.ok) ok++; else { err++; console.error(archivo?.nombre, res.error); }

      // Pausa entre requests para no saturar la API
      if (i < seleccionados.length - 1) await new Promise(res => setTimeout(res, 2000));
    }
    setMsg(`✅ Completado: ${ok} exitosos, ${err} errores. Recargando...`);
    setProcesando(false);
    setTimeout(() => cargarTodo(), 1500);
  };

  // Carga manual directa con file IDs separados por coma
  const [fileIdsManual, setFileIdsManual] = React.useState('');
  const procesarManual = async () => {
    const ids = fileIdsManual.split(/[\n,]+/).map(s => s.trim()).filter(Boolean);
    if (!ids.length) return;
    if (!consorcioActivo?.id) { setMsg('Seleccionar consorcio activo'); return; }
    setSeleccionados(ids);
    setArchivosEncontrados(ids.map(id => ({ id, nombre: `PDF ${id}` })));
    // trigger procesarSeleccionados via state update
    setProcesando(true);
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      setMsg(`⚙️ ${i+1}/${ids.length} — ${fileId}`);
      const colaId = `COLA-${consorcioActivo.id}-${fileId}`;
      await fetch(`${EF_URL}`, { method:'POST', headers:{'Content-Type':'application/json',Authorization:`Bearer ${tok}`}, body: JSON.stringify({ accion:'encolar_lote', archivos:[{ drive_file_id:fileId, drive_file_nombre:fileId, consorcio_id:consorcioActivo.id, consorcio_nombre:consorcioActivo.nombre }] }) });
      const r = await fetch(`${EF_URL}`, { method:'POST', headers:{'Content-Type':'application/json',Authorization:`Bearer ${tok}`}, body: JSON.stringify({ accion:'procesar_pdf', cola_id:colaId, pdf_id:fileId, pdf_url:`https://drive.google.com/file/d/${fileId}/view`, consorcio_id:consorcioActivo.id, consorcio_nombre:consorcioActivo.nombre }) });
      const res = await r.json();
      if (res.ok) ok++; else err++;
      if (i < ids.length - 1) await new Promise(res => setTimeout(res, 2000));
    }
    setMsg(`✅ ${ok} ok, ${err} errores`);
    setProcesando(false);
    cargarTodo();
  };

  // Cargar detalle de UFs de una liquidación
  const verDetalle = async (expensaId, periodo) => {
    ...
    setDetalle({ expensaId, periodo, items: Array.isArray(items)?items:[], ufs: Array.isArray(ufs)?ufs:[] });
    setTab('detalle');
  };
  ...
  const btn = (c='#1F4E79') => ({ background:c, color:'#fff', border:'none', borderRadius:7, padding:'8px 16px', cursor:'pointer', fontSize:13, fontWeight:600 });
  ...
  const input = { border:'1px solid #d1d5db', borderRadius:7, padding:'8px 12px', fontSize:13, width:'100%', boxSizing:'border-box' };

  // HISTORIAL agrupado por consorcio/período
  ...
  return (
    <div style={{ padding:20, maxWidth:1100, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:20, borderBottom:'2px solid #e5e7eb', paddingBottom:0 }}>
        {[
          { id:'importar', label:'📥 Importar', desc:'Cargar PDFs desde Drive' },
          { id:'historial', label:`📋 Importadas (${historial.length})`, desc:'Períodos ya procesados' },
          { id:'cola', label:`⚙️ Cola (${cola.length})`, desc:'Estado del procesamiento' },
          ...(detalle ? [{ id:'detalle', label:'🔍 Detalle', desc:'Rubros y UFs' }] : [])
        ].map(t => (
      ...
      {msg && <div style={{ background: msg.startsWith('✅')?'#d1fae5':msg.startsWith('❌')?'#fee2e2':'#fef9c3', border:'1px solid', borderColor:msg.startsWith('✅')?'#6ee7b7':msg.startsWith('❌')?'#fca5a5':'#fde68a', borderRadius:8, padding:'10px 16px', marginBottom:16, fontSize:13 }}>{msg}</div>}

      {procesando && (
        <div style={{ ...card, background:'#f0f9ff', border:'1px solid #bae6fd' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8 }}>
            <div style={{ width:18, height:18, border:'2px solid #1F4E79', borderTopColor:'transparent', borderRadius:'50%', animation:'spin 0.8s linear infinite' }} />
            <span style={{ fontWeight:600, color:'#1F4E79' }}>Procesando con IA...</span>
          </div>
          <div style={{ height:8, background:'#e2e8f0', borderRadius:4, overflow:'hidden' }}>
            <div style={{ height:'100%', background:'#1F4E79', borderRadius:4, width:`${(progreso.actual/progreso.total)*100}%`, transition:'width 0.5s' }} />
          </div>
          <div style={{ fontSize:12, color:'#6b7280', marginTop:6 }}>{progreso.actual} de {progreso.total} archivos procesados</div>
        </div>
      )}
      ...
      {tab === 'importar' && (
        <div>
          {!consorcioActivo && <div style={{ ...card, background:'#fef9c3', border:'1px solid #fde68a', color:'#92400e' }}>⚠️ Seleccioná un consorcio activo antes de importar</div>}

          {/* Modo 1: URL de carpeta Drive */}
          <div style={card}>
            <h3 style={{ margin:'0 0 12px', fontSize:15, color:'#374151' }}>🔗 Opción A — Carpeta de Drive</h3>
            <p style={{ fontSize:12, color:'#6b7280', margin:'0 0 12px' }}>Pegá la URL de la carpeta de Drive del edificio. El sistema detecta automáticamente los PDFs de liquidaciones.</p>
            <div style={{ display:'flex', gap:8 }}>
              <input style={{ ...input, flex:1 }} placeholder="https://drive.google.com/drive/folders/..." value={driveUrl} onChange={e=>setDriveUrl(e.target.value)} />
              <button style={btn()} onClick={buscarEnDrive} disabled={loading||!driveUrl}>🔍 Buscar</button>
            </div>
            {archivosEncontrados.length > 0 && (
              <div style={{ marginTop:16 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                  <span style={{ fontWeight:600, fontSize:13 }}>📄 {archivosEncontrados.length} archivos encontrados</span>
                  <div style={{ display:'flex', gap:8 }}>
                    <button style={btnOut} onClick={() => setSeleccionados(archivosEncontrados.map(a=>a.id))}>Seleccionar todos</button>
                    <button style={btnOut} onClick={() => setSeleccionados([])}>Deseleccionar</button>
                  </div>
                </div>
                <div style={{ maxHeight:220, overflowY:'auto', border:'1px solid #e5e7eb', borderRadius:7 }}>
                  {archivosEncontrados.map(a => (
                    <div key={a.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', borderBottom:'1px solid #f3f4f6', background: seleccionados.includes(a.id)?'#eff6ff':'#fff' }}>
                      <input type="checkbox" checked={seleccionados.includes(a.id)} onChange={e => setSeleccionados(prev => e.target.checked ? [...prev,a.id] : prev.filter(x=>x!==a.id))} />
                      <span style={{ fontSize:13, flex:1 }}>{a.nombre}</span>
                      <span style={{ fontSize:11, color:'#9ca3af' }}>{a.id}</span>
                    </div>
                  ))}
                </div>
                <div style={{ marginTop:12, display:'flex', justifyContent:'flex-end' }}>
                  <button style={btn('#107569')} onClick={procesarSeleccionados} disabled={procesando||!seleccionados.length||!consorcioActivo}>
                    🤖 Procesar {seleccionados.length} seleccionados con IA
                  </button>
                </div>
              </div>
            )}
          ...
          {/* Modo 2: IDs manuales */}
          <div style={card}>
            <h3 style={{ margin:'0 0 12px', fontSize:15, color:'#374151' }}>🔑 Opción B — IDs de Drive directos</h3>
            <p style={{ fontSize:12, color:'#6b7280', margin:'0 0 12px' }}>Pegá los IDs de Drive de los PDFs (uno por línea o separados por coma). El ID se obtiene de la URL del archivo: drive.google.com/file/d/<b>ID_AQUI</b>/view</p>
          ...
          <div style={{ ...card, background:'#f0fdf4', border:'1px solid #bbf7d0' }}>
            <h4 style={{ margin:'0 0 8px', fontSize:13, color:'#166534' }}>✅ ¿Qué hace la importación?</h4>
            <div style={{ fontSize:12, color:'#166534', lineHeight:1.7 }}>
              Por cada PDF de liquidación procesado, el sistema extrae con IA y carga automáticamente:
              <ul style={{ margin:'8px 0', paddingLeft:20 }}>
                <li><b>Período cerrado</b> en Liquidaciones con saldos financieros completos</li>
                <li><b>Ítems de gastos</b> por rubro (servicios, mantenimiento, administración, seguros, etc.)</li>
                <li><b>Proveedores</b> detectados automáticamente (nombre + CUIT si figura)</li>
                <li><b>Cuenta corriente histórica</b> por unidad funcional (saldo anterior, pagos, expensa, total)</li>
      ...
      {tab === 'historial' && (
        <div>
          <div style={{ display:'flex', gap:10, marginBottom:16, alignItems:'center' }}>
            <span style={{ fontSize:13, fontWeight:600 }}>Filtrar consorcio:</span>
            <select style={{ ...input, width:'auto' }} value={filtroCon} onChange={e=>setFiltroCon(e.target.value)}>
              <option value="todos">Todos ({historial.length})</option>
              {consorciosFiltro.map(id => <option key={id} value={id}>{consorcioPorId(id)?.nombre ?? id} ({historial.filter(h=>h.consorcio_id===id).length})</option>)}
            </select>
          </div>
          {loading ? <div style={{ textAlign:'center', padding:40, color:'#6b7280' }}>Cargando...</div> : historialFiltrado.length === 0 ? (
            <div style={{ ...card, textAlign:'center', color:'#6b7280', padding:40 }}>
              <div style={{ fontSize:40, marginBottom:12 }}>📂</div>
              No hay liquidaciones importadas aún.<br/>Usá la pestaña <b>Importar</b> para comenzar.
            </div>
          ) : (
            <div>
                ...
                return (
                  <div key={cid} style={{ ...card, padding:0, overflow:'hidden' }}>
                    <div style={{ background:'#1F4E79', color:'#fff', padding:'12px 20px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      ...
                      <span style={{ fontSize:12, opacity:0.8 }}>{perCon.length} período(s) importado(s)</span>
                    </div>
                    <table style={{ width:'100%', borderCollapse:'collapse' }}>
                      <thead>
                        <tr style={{ background:'#f8fafc' }}>
                          {['Período','Vencimiento','Total Gastos','Saldo Final','PDF','Acciones'].map(h=>
                      ...
                      <tbody>
                        {perCon.map((h,i) => (
                          <tr key={h.id} style={{ borderBottom:'1px solid #f3f4f6', background: i%2?'#fafafa':'#fff' }}>
                            <td style={{ padding:'10px 14px', fontSize:13, fontWeight:600 }}>{h.periodo}</td>
                            <td style={{ padding:'10px 14px', fontSize:12, color:'#6b7280' }}>{h.fecha_vencimiento ?? '—'}</td>
                            <td style={{ padding:'10px 14px', fontSize:13 }}>{h.total_gastos != null ? `$ ${Number(h.total_gastos).toLocaleString('es-AR')}` : '—'}</td>
                            ...
                            <td style={{ padding:'10px 14px' }}>
                              {h.drive_pdf_url ? <a href={h.drive_pdf_url} target="_blank" rel="noreferrer" style={{ color:'#1F4E79', fontSize:12 }}>📄 Ver PDF</a> : <span style={{ color:'#9ca3af', fontSize:11 }}>—</span>}
                            </td>
                            <td style={{ padding:'10px 14px' }}>
                              <button style={{ ...btnOut, fontSize:11, padding:'4px 10px' }} onClick={() => verDetalle(h.id, h.periodo)}>🔍 Rubros y UFs</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  {cola.map((c,i) => (
                    <tr key={c.id} style={{ borderBottom:'1px solid #f3f4f6', background:i%2?'#fafafa':'#fff' }}>
                      ...
                      <td style={{ padding:'8px 12px' }}>
                        ...
                        {c.error_mensaje && <div style={{ fontSize:10, color:'#dc2626', marginTop:2, maxWidth:200, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.error_mensaje}</div>}
                      </td>
      ...
      {tab === 'detalle' && detalle && (
        <div>
          <button style={{ ...btnOut, marginBottom:16 }} onClick={() => setTab('historial')}>← Volver al historial</button>
          ...
          <div style={{ ...card, marginBottom:16 }}>
            ...
            {detalle.items.length === 0 ? <div style={{ color:'#9ca3af', fontSize:13 }}>Sin datos de rubros</div> : (
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                <thead>
                  <tr style={{ background:'#f8fafc' }}>
                    {['Rubro','Proveedor','Concepto','N° Comprobante','Total'].map(h=>
                ...
                <tbody>
                  {detalle.items.map((it,i) => (
                    <tr key={it.id} style={{ borderBottom:'1px solid #f3f4f6', background:i%2?'#fafafa':'#fff' }}>
                      <td style={{ padding:'6px 10px' }}><span style={badge('#1F4E79')}>{it.rubro_nro}</span> {it.rubro_nombre}</td>
                      <td style={{ padding:'6px 10px' }}>{it.proveedor_nombre ?? '—'}{it.cuit_proveedor ? <span style={{ color:'#9ca3af', fontSize:10 }}><br/>{it.cuit_proveedor}</span> : null}</td>
                      <td style={{ padding:'6px 10px', maxWidth:200 }}>{it.concepto ?? '—'}</td>
                      <td style={{ padding:'6px 10px', color:'#6b7280' }}>{it.nro_comprobante ?? '—'}</td>
                      <td style={{ padding:'6px 10px', fontWeight:600, textAlign:'right' }}>$ {Number(it.importe_total).toLocaleString('es-AR')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Prorrateo UFs */}
          <div style={card}>
            <h4 style={{ margin:'0 0 12px', fontSize:14 }}>🏠 Cuenta Corriente por UF ({detalle.ufs.length} unidades)</h4>
            {detalle.ufs.length === 0 ? <div style={{ color:'#9ca3af', fontSize:13 }}>Sin datos de UFs</div> : (
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                  <thead>
                    <tr style={{ background:'#f8fafc' }}>
                      {['UF','Dpto','Propietario','Coef%','Saldo Ant.','Pagos','Interés','Expensa','Total'].map(h=>
                        <th key={h} style={{ padding:'6px 10px', textAlign:'right', color:'#6b7280', fontWeight:600, borderBottom:'1px solid #e5e7eb', textAlign: h==='Propietario'||h==='Dpto'?'left':'right' }}>{h}</th>
                      )}
                    </tr>
                  </thead>
                  <tbody>
                    {detalle.ufs.map((uf,i) => (
                      <tr key={uf.id} style={{ borderBottom:'1px solid #f3f4f6', background:i%2?'#fafafa':'#fff' }}>
                        ...
                        <td style={{ padding:'6px 10px', textAlign:'right', color:'#6b7280' }}>$ {Number(uf.saldo_anterior).toLocaleString('es-AR')}</td>
                        <td style={{ padding:'6px 10px', textAlign:'right', color:'#107569' }}>$ {Number(uf.pagos).toLocaleString('es-AR')}</td>
                        <td style={{ padding:'6px 10px', textAlign:'right', color:'#dc2626' }}>$ {Number(uf.interes).toLocaleString('es-AR')}</td>
                        <td style={{ padding:'6px 10px', textAlign:'right' }}>$ {Number(uf.expensa_calculada).toLocaleString('es-AR')}</td>
                        <td style={{ padding:'6px 10px', textAlign:'right', fontWeight:700, color:(uf.total_uf??0)>=0?'#107569':'#dc2626' }}>$ {Number(uf.total_uf).toLocaleString('es-AR')}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export default function App() {
  ...
  const NAV=[
    ...
    { id:'historial_liquidaciones', label:'Historial Liquidaciones', icon:'📂', sec:'Reportes' },
